...
Two rows satisfy both conditions, and each names its citation. `src/Billing/BillingProcessor/X12RemoteTracker.php`, composed score 4, on `src/Billing/BillingProcessor/X12RemoteTracker.php:L112-L121`, where a failed upload is overwritten with the success status and the status column is the only place the transport records a verdict. `src/Billing/SLEOB.php`, composed score 6, on `src/Billing/SLEOB.php:L41-L42`, where payer-supplied data is interpolated into a query in a routine that has no validation and no error branch, so one of its three possible outcomes is a payment worked against an encounter the remittance did not name, with nothing raised. Escalated rows are marked as such in the table so that the arithmetic and the override are never confused with each other. Both underlying defects are registered, with their symptoms and their proposed verifications, in [defect-candidates.md](defect-candidates.md).

Two rows that a reader might expect to be escalated are not, and in each case the reason is one of the two conditions rather than a change of view about the file. `src/Billing/Claim.php` reaches 7 on its own arithmetic once its narrow coverage is scored as narrow, so it fails the second condition and escalation would add nothing to it. `library/edihistory/edih_835_html.php` also reaches 7 on its own arithmetic, and it fails the first condition as well: the defect that makes it interesting is a segment it renders and the modern parser rejects, and the rejecting line is cited in `src/Billing/ParseERA.php` rather than in this file.

`src/Billing/SLEOB.php` needs one further note, because it is escalated on one region while a second region in the same file does **not** qualify, and the difference is what shows the rule is being applied rather than reached for. The region it is escalated on is the payer-supplied interpolation at `src/Billing/SLEOB.php:L41-L42`, in a routine with no validation and no error branch. The region it is **not** escalated on is the tertiary-payer boundary at `src/Billing/SLEOB.php:L285-L288`, which is a genuine defect cited in the file itself and still fails the **first** condition on the operator-visible half of it. VERIFIED: the only caller that reaches that routine invokes it inside the guard at `interface/billing/sl_eob_process.php:L717`, and then prints a message whenever the crossover test at `interface/billing/sl_eob_process.php:L720` is not met - a fixed sentence about secondary paper billing, at `interface/billing/sl_eob_process.php:L721-L725`. The message does not depend on what the routine did, so a signal always reaches the screen and a tertiary requeue is *misreported* rather than unreported - and misreporting is a different fault from silence, however unhelpful it is in practice. Rule E as written asks whether a signal exists, not whether it is accurate, so that region on its own would have left the row at `caution`. Widening rule E to cover misleading signals as well as absent ones would be a defensible method but a different one, and rewriting the rule to reach a row is precisely the move this document's published-arithmetic approach exists to prevent - so the rule is unchanged and the escalation rests only on the region that meets it. The row's evidence is set out in full at [src/Billing/SLEOB.php](#srcbillingsleobphp), and the misleading message is registered as a rule in [business-rules.md](business-rules.md) and as a defect in [defect-candidates.md](defect-candidates.md).
...
**What that group's coverage cells do and do not establish.** The evidence behind those four cells is the existence of a dedicated test that exercises behaviour, and nothing stronger, so the phrase to use for them is that each has a dedicated test rather than that each is fully covered. VERIFIED: no coverage report was produced for this document, because nothing was executed - PHP, Composer and the test suites are not installed in the authoring environment, as the attribution section records - so no statement here rests on measured line or branch coverage. VERIFIED: the largest of the four shows why the distinction is not pedantic. `src/Billing/EdiHistory/X12File.php` declares **22 explicitly public methods** between `src/Billing/EdiHistory/X12File.php:L191` and `src/Billing/EdiHistory/X12File.php:L1220`, and **24 public methods in the class overall**, because two further members carry no visibility keyword and are therefore public by default: the constructor at `src/Billing/EdiHistory/X12File.php:L103` and a segment-slicing helper at `src/Billing/EdiHistory/X12File.php:L1316`. Its 415-line test declares 25 test methods and names four of those 24 nowhere at all - the envelope walker at `src/Billing/EdiHistory/X12File.php:L580`, the segment splitter at `src/Billing/EdiHistory/X12File.php:L905`, the segment finder at `src/Billing/EdiHistory/X12File.php:L1220` and the segment-slicing helper at `src/Billing/EdiHistory/X12File.php:L1316` - and a search of `tests/Tests/Isolated/Billing/EdiHistory/X12FileIsolatedTest.php` for each of those four names returns nothing. The constructor is the one public member exercised without being named: VERIFIED, the test instantiates the class 25 times between `tests/Tests/Isolated/Billing/EdiHistory/X12FileIsolatedTest.php:L87` and `tests/Tests/Isolated/Billing/EdiHistory/X12FileIsolatedTest.php:L383`, eight of them in the first block at `:L87-L156`, and exactly one of the 25 passes all three constructor arguments, at `:L156`; the last instantiation, at `:L383`, passes only the path. That is why a name census is the right instrument for a named method and the wrong one for a constructor. Those cells are therefore evidence that a change breaking the tested paths would be caught, not evidence that a change anywhere in the class would be.
...
Only the 18 high-risk rows carry a subsection, and that is a statement about depth rather than about safety. A `caution` classification is not a clean bill of health: the band means one or two signals are adverse and that a change there needs a specific verification argument, as [How the four signals compose into a classification](#how-the-four-signals-compose-into-a-classification) defines it. The 34 caution rows carry their evidence in their four table cells, and the absence of a subsection below should be read as the absence of a fifth thing worth saying about them rather than as reassurance.
...
VERIFIED: 576 lines across 6 functions, at `library/edihistory/edih_uploads.php:L22`, `:L56`, `:L88`, `:L178`, `:L288` and `:L502`. This is the **EDI History** upload boundary, and it is one of five independent inbound boundaries rather than the subsystem's only one - a distinction that matters because a maintainer's untrusted-input model has to cover all five. VERIFIED: this file is reached from exactly two places, both of them on the EDI History path. The operator-facing screen loads it at `interface/billing/edih_main.php:L74`, and the legacy input router calls into it at `library/edihistory/edih_io.php:L287` and `:L529`; both entries land on `edih_upload_files()` at `library/edihistory/edih_uploads.php:L288`, which is also where the file inspects the request at `:L294`. Nothing else in the repository references it, which is what the coupling count of 2 records.

VERIFIED: four further paths carry payer or clearinghouse data into the subsystem without touching this file, and each one hands it to a parser directly. A 271 eligibility response uploaded on the eligibility screen is read from `$_FILES`, written to the configured path and passed straight to the modern parser, at `interface/billing/edi_271.php:L42-L65`, with the parse call itself at `:L65`. An 835 remittance uploaded on the ERA screen is read from `$_FILES` and passed straight to `ParseERA::parseERA()`, at `interface/billing/era_payments.php:L117-L144`, with the parse call at `:L144` and the file only afterwards moved into the ERA directory at `:L156-L170`. A second 835 upload lives on the EOB search screen and reaches the same parser with a different callback, at `interface/billing/sl_eob_search.php:L965-L992`, with the parse call at `:L992`; the two screens share their archive-extraction step and differ in everything around it, as the next paragraph sets out. And a real-time 271 arrives over HTTP rather than as a file at all: the response body is split as a multipart payload at `src/Billing/EDI270.php:L852-L858`, the payload is returned at `:L875`, and the accumulated payloads are parsed as X12 at `src/Billing/EDI270.php:L509`, reached from the eligibility screen at `interface/billing/edi_270.php:L167`. The risk in this file is therefore that it handles input the system does not control, on a path with no test, in a file whose behaviour was last deliberately changed in 2018 - not that it is the only place such input arrives.

VERIFIED: what those two screens actually share is the archive-extraction block and nothing else - `interface/billing/era_payments.php:L126-L142` and `interface/billing/sl_eob_search.php:L972-L988` are the same extension test, the same read of the first member, the same overwrite of the temporary file and the same three error strings - so a change to extraction has two sites to reach, and that is the only claim of similarity this document makes. VERIFIED: everything around that block differs, in five ways a refactor has to account for. The token is checked inside the upload branch on the ERA screen, at `interface/billing/era_payments.php:L118`, whereas on the EOB search screen it is checked by the enclosing search-or-print branch, at `interface/billing/sl_eob_search.php:L951-L953`, so the upload there is reached only as part of a search submission. The ERA screen creates the destination directory when it is absent, at `interface/billing/era_payments.php:L151-L152`; the EOB search screen composes the same path at `interface/billing/sl_eob_search.php:L998` and renames into it without creating it. On a name collision the ERA screen keeps both files, renaming the upload to a hidden pending name at `interface/billing/era_payments.php:L160-L161` and setting a confirmation flag at `interface/billing/era_payments.php:L163`, while the EOB search screen appends a warning at `interface/billing/sl_eob_search.php:L1002-L1007` and then renames over the existing file regardless, at `interface/billing/sl_eob_search.php:L1009`. The EOB search screen additionally copies the stored file into the EDI History 835 directory at `interface/billing/sl_eob_search.php:L1010`, which the ERA screen never does - the reason a remittance taken in on the ERA screen is absent from the history index, as [claim-lifecycle.md](claim-lifecycle.md) records for stage S13. And the two compose the stored name from different inputs: the ERA screen from the payer identifier alone at `interface/billing/era_payments.php:L65-L66`, the EOB search screen falling back to the interchange sender identifier when the payer identifier is empty at `interface/billing/sl_eob_search.php:L181-L182`.

VERIFIED: two properties of that 835 intake belong in a maintainer's input model, because both sit on the shared block or on the path immediately around it. The stored name is composed from values the payer sends - the functional-group date, the interchange control number and the payer identifier, read by the parser at `src/Billing/ParseERA.php:L131-L133`, `src/Billing/ParseERA.php:L140-L142` and `src/Billing/ParseERA.php:L186-L188`, the last of them overwritten from a reference segment at `src/Billing/ParseERA.php:L196-L199` - and that name is what both screens interpolate into the destination path. And the archive member is read whole into memory at `interface/billing/era_payments.php:L129` and `interface/billing/sl_eob_search.php:L975` and written back to the temporary file at `interface/billing/era_payments.php:L134` and `interface/billing/sl_eob_search.php:L980`, with no uncompressed-size and no member-count budget on either screen.

VERIFIED: on the narrower question of how this file itself handles an upload, it is the most defensive of the five rather than the least, which is worth knowing before anyone refactors it. It detects the request-size overflow in which both arrays arrive empty, at `library/edihistory/edih_uploads.php:L294-L299`; it refuses a request with no file array at `:L301-L304`; it accepts only two expected array keys at `:L307-L314`; it rejects a malformed error member, with the comment at `:L318` naming array-corruption abuse as the reason, at `:L317-L324`; and it applies both a media-type allowlist at `:L329` and an extension denylist at `:L332`, the comment above the former conceding at `:L327-L328` that media type is not reliable. A refactor that consolidated the five boundaries onto one path would need to carry those checks forward, because four of the five do not have them.

Where this reading encountered a security-sensitive observation on these boundaries it is flagged, one line and one severity and no further analysis, in [Appendix A of defect-candidates.md](defect-candidates.md#appendix-a-security-sensitive-observations): the 271 eligibility upload as item A16, the payer-derived stored name on both 835 screens as item A17, the unbounded archive expansion on both as item A18, and the real-time eligibility path as item A9. There is no row for this file's own upload handling, for the reason just given. That appendix states its own scope, which is the paths this documentation set names and no wider, and it is explicit that the absence of a row is not evidence that a path was assessed and found sound.
...
304 lines, coverage `none`, inbound coupling 9, 19 commits, last substantive change 2026-02-09 (`4ca569023`). Composed score 6, the top of the `caution` band on its arithmetic, and **escalated** to `high-risk` under rule E on the first of the four regions below. The rule and its two conditions are in [How the four signals compose into a classification](#how-the-four-signals-compose-into-a-classification); the specific evidence is here.

This is the accounts-receivable poster, so every line in it is one line from a patient- or payer-facing dollar amount, and four regions each independently justify treating a change here as consequential. Read the band as a statement about the evidence rather than about the stakes: the arithmetic puts the row at the top of `caution`, and what carries it into `high-risk` is a cited silent-money region rather than a judgement that money makes any change dangerous. The escalation rests on the first of the four regions below and on nothing else: rule E is applied here exactly as published, so the other three are risk evidence without being escalation grounds, and the third of them is worth reading precisely for why it does not qualify.

VERIFIED: `src/Billing/SLEOB.php:L41-L42` builds a query in which the patient identifier is interpolated into the statement text while the value beside it is bound as a parameter. The inconsistency within a single statement is the finding, and the data reaching it comes from a remittance file the payer supplies. The security dimension of it is flagged, not analysed, as item A1 of [Appendix A of defect-candidates.md](defect-candidates.md#appendix-a-security-sensitive-observations). What matters for the classification is the range of outcomes, and one of them is silent. VERIFIED: the routine has no validation and no error branch of any kind. `slInvoiceNumber()` runs from `src/Billing/SLEOB.php:L28` to `:L64`; it splits the payer-supplied claim identifier at `:L31` without checking any part of it, the three-part case assigns whatever the query returned at `:L44`, and when nothing was returned the guard at `:L59` simply fails and the routine returns the unmodified identifier at `:L63`. Nothing anywhere in it logs, throws or reports. So the possible outcomes are three: a query that errors, a query that selects a row other than the intended one, and a silent misattribution in which a remittance line is worked against an encounter the remittance did not name. VERIFIED: the consumer does not close that third outcome. The posting screen destructures the return at `interface/billing/sl_eob_process.php:L325` and re-checks the pair at `:L332-L335`, but that check is an existence test against `form_encounter` and `patient_data` and not a comparison against the patient the remittance itself names, so any pair resolving to some real encounter passes it and only an unresolvable pair is reset at `:L336-L338`. That is a silent-money defect cited to a line range in this same file, which is rule E's first condition, and the composed score of 6 is below 7, which is its second - so the row is escalated. Rule E was not widened to reach it: the region satisfies the rule exactly as the method section states it. The silent-misattribution defect is registered with its observable symptom and its proposed verification in [defect-candidates.md](defect-candidates.md), which is where rule E requires it to be; the injection dimension of the same two lines is flagged separately and without analysis as item A1 of that document's Appendix A, and nothing here analyses it further.
...
VERIFIED: `src/Billing/SLEOB.php:L285-L288` decides which payer to advance a claim to by converting the encounter's billed-level watermark to a number and then testing it with a mixed `&&` and `||` condition that is not parenthesised, so the grouping is determined by operator precedence rather than stated. This is the tertiary-payer boundary, and it is the region a reader would expect to be the escalation ground. It is not, and the reason is narrow and worth stating exactly, because the row is escalated on the region above rather than on this one. VERIFIED: the caller invokes this routine inside the guard at `interface/billing/sl_eob_process.php:L717` and then prints a fixed sentence about secondary paper billing at `interface/billing/sl_eob_process.php:L721-L725` whenever the crossover test at `:L720` is not met, without consulting what the routine did. A signal therefore always reaches the operator, and a claim requeued to the tertiary payer that just paid it is reported as though it had advanced to secondary. Rule E asks whether a signal exists, not whether it is correct, so this region alone would have left the row at its arithmetic band - but a reader planning a change here should treat the reported outcome as unreliable rather than absent. The rule is registered in [business-rules.md](business-rules.md) and the defect in [defect-candidates.md](defect-candidates.md).
