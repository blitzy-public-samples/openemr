...
VERIFIED: on the narrower question of how this file itself handles an upload, it validates the request more heavily than any of the other four intake paths - six successive checks between `library/edihistory/edih_uploads.php:L294` and `library/edihistory/edih_uploads.php:L332`, where the most any other path applies is three, on the 271 screen at `interface/billing/edi_271.php:L46-L55`. That is worth knowing before anyone refactors it: a refactor that consolidated the five boundaries onto one path would have more to carry forward from here than from anywhere else, and that is the risk-relevant fact. What each of the six checks does is not analysed in this document.
