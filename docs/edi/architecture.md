...
[README.md](README.md) records that this documentation run catalogued thirteen instances in the subsystem where descriptive text contradicts the code it describes, and defers the analysis of them to this document and to [defect-candidates.md](defect-candidates.md). Eight of them are in the legacy tree and are listed below; the remaining five are outside it and are anchored in [README.md](README.md). Five of the eight are of one kind: a file whose own docblock names a different file than the one it is in. The other three are each of a different kind - a docblock naming the wrong transaction type for the function directly beneath it, a comment announcing an access guard that is not in force, and a docblock attributing behaviour to the function it introduces when a different function performs it. They are recorded here because they are the direct evidence for the source-of-truth ordering, not because any of them changes behaviour.
