...
The ordering is reinforced structurally by what the schema does not contain. `sql/database.sql` runs to 15,395 lines, ending at `sql/database.sql:L15395`, and declares 283 tables; that figure is a case-insensitive enumeration of `CREATE TABLE` at the recorded commit which tolerates leading whitespace and any run of whitespace between the two keywords, and two lower counts are easy to arrive at and both understate it. A pattern anchored to the start of a line returns 281, because one statement is indented (`sql/database.sql:L1180`, `contact_address`). A pattern that allows the indentation but requires a single space between the keywords returns 282, because one statement is written with two (`sql/database.sql:L1778`, `erx_ttl_touch`). No occurrence of the phrase appears anywhere but at the head of a statement, so nothing is gained by anchoring and 283 is the count used throughout this documentation set. Across all 283 there are no `CHECK` constraints, no declared foreign keys, no triggers and no views. The two tables at the centre of this subsystem show the shape - each closes on a primary key and, at most, one secondary index, with nothing else between the last column and the storage-engine clause (`sql/database.sql:L245-L278` for `billing`, `sql/database.sql:L378-L393` for `claims`). Several invariants of this subsystem are therefore recorded only as column `COMMENT` text, as the claim version is when it states that it is incremented in code (`sql/database.sql:L381`), which makes those comments first-class evidence of a rule. That is not a licence to trust a comment about behaviour: a comment is admissible for what an invariant was meant to be and never for what the code does. The schema shows the difference itself. The phrase "foreign key" appears in it four times, and every occurrence is inside a column comment describing a relationship the database does not enforce (`sql/database.sql:L15138-L15139` and `sql/database.sql:L15155-L15156`).
...
- **21** database tables read or written by in-scope code. A table counts here when a statement inside one of the 67 in-scope files names it and the row it touches is revenue-cycle data - a charge, a claim, an encounter, a coverage row, a payer, a trading partner, a ledger line or a deposit. Fifteen of the 21 are named in no existing document, and two of those fifteen are referenced by this code more often than either `claims` (`sql/database.sql:L378`) or `ar_activity` (`sql/database.sql:L10188`): `users`, whose DDL is at `sql/database.sql:L9786` and which is read at `src/Billing/Claim.php:L132` and `src/Billing/MiscBillingOptions.php:L83` among others, and `insurance_data`, whose DDL is at `sql/database.sql:L3306` and which is read at `src/Billing/BillingUtilities.php:L1771` and `src/Billing/SLEOB.php:L258`. Every one of the 21 is anchored to its DDL, with its reference count, in the stage-by-table matrix in [claim-lifecycle.md](claim-lifecycle.md), and 21 is the denominator that matrix uses. **Ten further stored objects** are reached while those stages run without holding revenue-cycle data - by direct reference outside that surface, by transitive reach through a service or model, or as general infrastructure such as identifier allocation and key storage. They are neither counted in the 21 nor omitted: the same document carries them in a separately titled supplementary inventory that records the route, the stage and the statement for each. Where a figure of 31 appears for this subsystem it is those two sets added together, and it is not the canonical count.
