...
| 4 | Boilerplate description in `src/Billing/` | `src/Billing/BillingUtilities.php:L4` carries the identical sentence, in a block that also opens `/*` (`src/Billing/BillingUtilities.php:L3-L13`) | The sentence fits this file, but it heads five files in total, the other three being `interface/billing/era_payments.php:L4`, `interface/billing/new_payment.php:L6` and `interface/billing/edit_payment.php:L6`, so it was authored for none of them and identifies none of them |
