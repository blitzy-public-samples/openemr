...
|-----------|--------------------------------------|-------------|---------------|
| Outbound batch | `documents/edi` | `src/Billing/BillingProcessor/BillingClaimBatch.php:L65` | Generated claim batch files, and - the one thing in it that is not outbound - the encrypted 271 archives written by the eligibility upload screen, per `interface/globals.php:L368` and `interface/billing/edi_271.php:L44` |
...
- The eligibility path is documented in full, because it is an X12 path and therefore squarely inside this document's subject.
...
VERIFIED: **the 271 upload does write a file, and it writes it outside the history tree.** The destination directory is the `edi_271_file_path` global, set to the site's `documents/edi/` at `interface/globals.php:L368` and read at `interface/billing/edi_271.php:L38`. The stored name is that directory, then the current Unix timestamp, then the uploaded file's base name, concatenated at `interface/billing/edi_271.php:L44`, so each upload lands as `documents/edi/<timestamp><originalname>` and no upload can overwrite another. The bytes are passed through the filesystem encryption accessor before being written, at `interface/billing/edi_271.php:L59-L60`, and the file is read straight back and passed through the matching decryption accessor in the same request in order to parse it, at `interface/billing/edi_271.php:L62-L65`. VERIFIED: whether that archive is actually encrypted depends on configuration rather than on the code path - the accessor returns its argument unchanged when the `drive_encryption` global is off, at `src/BC/Crypto/ContextualEncryptionTrait.php:L36-L37` against the flag set at `src/Common/Crypto/CryptoGen.php:L83-L84` - so a site with that setting off stores the payer's response in clear. Nothing ever deletes the file either way, so this directory accumulates one archived response per upload.
...
**Failure modes and operator-visible symptoms.** Five: one on the fee sheet path and four on the 271 upload.
...
Internally, a stored file that decrypts to nothing is reported as having failed to open, at `interface/billing/edi_271.php:L66-L68`, and the parse is skipped. VERIFIED: that message overwrites the success message already assigned at `interface/billing/edi_271.php:L61`, so the operator sees only the failure - but the encrypted file has already been written and stays there.
...
**Tables read.** Twenty-three, which is more than any other stage reads and needs unpacking in three parts. Thirteen are read in building the claim, all through the claim model unless noted, and they are the table below; a fourteenth is read on the institutional path only and a fifteenth on the write path, and both follow that table. The remaining eight are reached transitively, through services and models that the claim model constructs rather than through any statement in `src/Billing/`, and they are enumerated with their statements under [The Stage by Table Matrix](#the-stage-by-table-matrix).
...
**A scope correction first, because the count is not what it is usually assumed to be.** The revenue cycle is commonly described as touching six tables. VERIFIED: following executable control flow out of the entry points these stages name reaches **thirty-one** stored objects, and every one of them is anchored below to the data definition statement that declares it. Twenty-five of the thirty-one are additions to the usual six, and two of the additions outrank two of the originals: `users` and `insurance_data` carry more query sites in `src/Billing/` than either `claims` or `ar_activity` does.

**What this matrix is exhaustive for, and what it is not.** It is exhaustive for the stored objects that executable control flow reaches from the entry points named in S1 through S13, at the depth of the call graph those stages document. It is not an inventory of every object a running instance touches while a claim moves through it, and the difference is a declared boundary rather than an oversight. Three categories are excluded, and they are named here so that a dot in this table is read as "not reached by the traced path" rather than as "not touched at all".
...
VERIFIED: crossing the first of those boundaries alone raises the count from thirty-one to thirty-four, by the three audit objects just cited.
...
- VERIFIED: `ar_session`, `ar_activity` and `code_types` are the only three of the thirty-one declared without backtick quoting around the table name, at `sql/database.sql:L10158`, `sql/database.sql:L10188` and `sql/database.sql:L10596`. The other twenty-eight are quoted. VERIFIED: the difference is cosmetic in effect, because the unquoted names are not reserved words and the statements parse identically.
...
VERIFIED: a reference count of zero is a finding rather than a gap in the inventory. It means no file in `src/Billing/` names that object in a statement of its own, and the object is reached only through a service, library class or helper that `src/Billing/` calls. Nine of the thirty-one rows are in that position, and they are the reason the usual six-table description of this subsystem survives: a reader grepping `src/Billing/` for table names cannot see them at all.
...
| `list_options` | 2 | `sql/database.sql:L3887` | . | RW | . | . | R | . | . | . | . | . | . | . | . | . |
...
| `addresses` | 0 | `sql/database.sql:L14` | . | . | . | . | R | . | . | . | . | . | . | . | . | . |
| `edi_sequences` | 0 | `sql/database.sql:L1635` | . | . | . | RW | . | . | . | . | . | . | . | . | . | . |
| `insurance_type_codes` | 0 | `sql/database.sql:L3372` | . | . | . | . | R | . | . | . | . | . | . | . | . | . |
| `keys` | 0 | `sql/database.sql:L3511` | RW | . | . | . | . | . | R | . | . | . | . | . | . | . |
| `openemr_postcalendar_categories` | 0 | `sql/database.sql:L8192` | . | . | . | . | R | . | . | . | . | . | . | . | . | . |
| `phone_numbers` | 0 | `sql/database.sql:L8651` | . | . | . | . | R | . | . | . | . | . | . | . | . | . |
| `uuid_mapping` | 0 | `sql/database.sql:L9937` | . | . | . | . | R | . | . | . | . | . | . | . | . | . |
| `uuid_registry` | 0 | `sql/database.sql:L9959` | . | . | . | . | RW | . | . | . | . | . | . | . | . | . |
| `valueset` | 0 | `sql/database.sql:L12927` | . | . | . | . | R | . | . | . | . | . | . | . | . | . |

**Where the transitive rows come from.** Seven of the nine zero-reference rows are reached in S4 through the claim data model, and one further S4 read lands on `list_options`, which carries reference counts of its own from S1. VERIFIED: constructing a claim runs the service cascade at `src/Billing/Claim.php:L63-L91`, and constructing the payer model runs a second cascade from `src/Billing/Claim.php:L289`. VERIFIED: the first three rows below are reached because the encounter fetch at `src/Billing/Claim.php:L66` is not a simple row read - `getOneByPidEid()` delegates to the full search at `src/Services/EncounterService.php:L109`, and that search carries the joins listed here. The reads those two cascades add are these.

| Object | Reached through | Statement |
|--------|-----------------|-----------|
| `openemr_postcalendar_categories` | the encounter search called from `src/Billing/Claim.php:L66` | `src/Services/EncounterService.php:L263-L264` |
| `list_options` | the same search, twice: the visit-class join and the discharge-disposition subquery | `src/Services/EncounterService.php:L265` and `src/Services/EncounterService.php:L310-L315` |
| `uuid_mapping` | the same search, twice, as the location alias | `src/Services/EncounterService.php:L273-L274` and `src/Services/EncounterService.php:L307-L308` |
| `addresses` | the payer model's `populate()`, at `library/classes/InsuranceCompany.class.php:L321` | `src/Common/ORDataObject/Address.php:L68`, on the table named at `src/Common/ORDataObject/Address.php:L43` |
| `phone_numbers` | the same `populate()`, at `library/classes/InsuranceCompany.class.php:L324` | `src/Services/PhoneNumberService.php:L152` |
| `insurance_type_codes` | the payer model's constructor, at `library/classes/InsuranceCompany.class.php:L94` and `library/classes/InsuranceCompany.class.php:L95` | `src/Services/InsuranceCompanyService.php:L301` and `src/Services/InsuranceCompanyService.php:L322` |
| `valueset` | the payer model's constructor, at `library/classes/InsuranceCompany.class.php:L101` | `src/Services/InsuranceCompanyService.php:L342-L344` |
| `uuid_registry` | the identifier backfill described next | `src/Common/Uuid/UuidRegistry.php:L383` and `src/Common/Uuid/UuidRegistry.php:L539` |

The two remaining zero-reference rows are not part of that cascade: `edi_sequences` belongs to S3 and is described at the end of this section, and `keys` belongs to S0 and S6 and is described in both of those stages.
...
|-------|----------------|----------------------------------|
| S0 | None beyond the charge itself, because this worked claim carries no copay. A copay would write `ar_session` and `ar_activity` here, before any claim exists, per [S0](#stage-s0-encounter-and-fee-sheet-entry) | None. A 271 upload would write one encrypted archive into the **outbound batch directory**, but this claim's coverage was already on file |
...
| S3 | `edi_sequences` is read and advanced twice, once for the interchange control number and once for the group control number, when the generating task constructs its batch. No claim state changes: with a generating task selected this stage otherwise only reads `x12_partners` for the processing format and builds the claim objects, and the two non-generating tasks are the ones that write claim state here | None |
