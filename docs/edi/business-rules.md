...
**Transaction set numbers.** **837** is a health care claim, and it is one transaction set rather than two: **837P** and **837I** are its two implementation-guide variants, the professional claim submitted by individual practitioners and the institutional claim submitted by facilities. Both are written here with their variant letter, because every rule below that touches claim generation is specific to one variant. **835** is remittance advice, a payer's statement of what it paid and why, also called an **ERA** for electronic remittance advice; the human-readable equivalent is an **EOB**, an explanation of benefits. **270** and **271** are an eligibility request and its response. **276** and **277** are a claim-status inquiry and its response. **278** is a services review, that is, an authorisation. **997** and **999** are two distinct acknowledgement transaction sets rather than one: the 997 functional acknowledgement reports acceptance or rejection against the generic X12 syntax, and the 999 implementation acknowledgement is its 5010-era replacement, reporting errors against an implementation guide as well.
...
Seventy-two rules. Every one appears once in this table and once as an entry below.
...
| [BR-I6](#br-i6-one-switch-decides-what-the-interface-offers-and-which-payer-identifier-the-request-carries) | One switch decides what the interface offers and which payer identifier the request carries | I | VERIFIED | n/a |
...
That is `src/Billing/X125010837P.php:L665-L667`. The total starts as the integer zero and is accumulated once per procedure, so the addition is the only place the individual line charges meet. VERIFIED: the source value has already passed through the identity cleaner described in [BR-G6](#br-g6-values-passed-through-the-cleaner-are-uppercased-and-filtered-to-a-fixed-character-set), so it reaches `floatval()` as an uppercased, character-filtered string rather than as a number.
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** Dropping the exclusion subtracts the patient's share a second time, because `src/Billing/ParseERA.php:L41` has already removed the claim-level patient-responsibility total. The adjustment total therefore ends negative by exactly the patient's share, and that negative figure becomes the amount of the synthetic adjustment created by [BR-B4](#br-b4-an-artificial-service-line-named-claim-absorbs-the-residue) at `src/Billing/ParseERA.php:L71`. A negative adjustment is not a write-off. The synthetic adjustment carries the group code `CR` at `src/Billing/ParseERA.php:L69`, so the posting screen never reads it as a contractual write-off, and one of two branches receives it. Where the recomputed payment total rounds to zero, so the synthetic line carries no payment, the strict test at `interface/billing/sl_eob_process.php:L636-L639` raises the error flag and posts nothing further for the claim. Where the payment total is also out of balance, so the synthetic line carries a non-zero payment, the negative amount is posted as an adjustment at `interface/billing/sl_eob_process.php:L642-L652` and then subtracted from the running invoice total at `interface/billing/sl_eob_process.php:L653`, which increases the balance the invoice still shows as owed.
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** Removing either rounding call makes the balancing rewrite fire whenever the accumulated subtractions at `src/Billing/ParseERA.php:L43-L48` leave a residue that is not exactly zero as a binary floating-point value. That is a wider condition than the remittance being genuinely out of balance, because it also catches remittances that do balance to the cent but whose float arithmetic did not cancel exactly. When it fires, the rewrite inserts a synthetic service line and a synthetic adjustment worth that residue into the ledger. How often the condition would be met is not determinable from the code, because it depends on the particular cent amounts a given remittance carries and on the order in which they are subtracted. The tests at `src/Billing/ParseERA.php:L53` are loose comparisons against the integer zero, so the rounding is what makes them mean what they appear to mean.
...
That is `src/Billing/ParseERA.php:L401-L402`, the comment being the one the Intent field reads. The reason-code comparison is strict while the group-code comparison beside it is loose, so the exemption requires the code to arrive as the exact three-character string, and the sign flip the condition guards is at `src/Billing/ParseERA.php:L405`.
...
- **Statement:** Before anything is posted for a service line, the amount the payer says was charged is compared with the amount on the practice's own invoice, to the cent. If they differ, an error flag is raised. The flag suppresses posting for that service line and for every service line after it, but it does not reverse the service lines of the same claim that were already posted before it was raised, and no transaction encloses the loop. What a claim is left holding therefore depends on the position of the mismatched line within its own service list.
- **Evidence:** `interface/billing/sl_eob_process.php:L483-L492`, with the comparison at `interface/billing/sl_eob_process.php:L484-L485` and the error flag at `interface/billing/sl_eob_process.php:L491`. The loop the flag governs runs from `interface/billing/sl_eob_process.php:L449` to `interface/billing/sl_eob_process.php:L668`; the payment posting it guards is at `interface/billing/sl_eob_process.php:L558-L591` and the adjustment posting at `interface/billing/sl_eob_process.php:L621-L653`. No transaction is opened anywhere along that path, and the post-loop cleanup at `interface/billing/sl_eob_process.php:L696-L728` is skipped once the flag is set.
- **Status:** VERIFIED
...
That is `src/Billing/ParseERA.php:L419-L421`. The general arm sits immediately below it, at `src/Billing/ParseERA.php:L422-L423`, and differs only in the qualifier test, which is what makes the exception easy to miss: the general case is the second branch, and the qualified case above it is the only amount segment in the parser whose value survives.
...
- **Blast radius:** The comparison counts payments only and ignores adjustments, and it excludes soft-deleted lines, so a deposit whose payments were voided per [BR-F10](#br-f10-a-voided-receipt-is-journalled-only-under-one-configuration) reports as short even though the void was intentional. Because the only output is a client-side alert, an operator who has navigated away, or whose browser suppressed the dialog, receives no indication at all, and no later screen recovers this verdict. Two later screens do compute a related figure from the same rows, the payment edit screen at `interface/billing/payment_master.inc.php:L89` and the payment search screen at `interface/billing/search_payments.php:L575-L576`, but both subtract the deposit's global allocation as well, so neither reproduces what this check reported. The dry-run guard means the check never runs in the mode an operator would use to preview a file.
...
- **Statement:** A service line that the payer adjusted without paying anything is normally treated as an error, which blocks posting for that line and for every line after it in the same claim. Two adjustment reason codes are exempt when the payer marked them as a contractual obligation, and a line carrying one of them posts with no payment and no error.
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** The exempt list is a literal of exactly two codes compared strictly. Any other reason code that legitimately pays nothing raises the error flag, which stops that service line and every service line after it from posting while leaving in place whatever the same claim posted before it, because no transaction encloses the loop at `interface/billing/sl_eob_process.php:L449-L668`; the shape of that outcome is registered in [BR-A9](#br-a9-a-charge-that-disagrees-with-the-invoice-by-one-cent-blocks-the-posting). The operator sees a refusal on the offending line rather than a write-off recorded. Widening the list allows more zero-payment lines to post unremarked, which is the same silence in the other direction.
...
- **Statement:** Internal claim status 7 means the claim was denied, and it is the only status handled by its own branch at every step of the claim update. A denial writes the status value into the claim row and into the queue's process column, and stores the denial reason in the claim's process-file column. It leaves the charge row's billed flag and billed date untouched, which is not the same as clearing them: a claim that was already sent still reads as billed after it is denied. The columns a denial declines to write are written by the general branch on a different rule again, since that branch marks any status above 1 as billed but stamps the billed date for status 2 alone.
...
- **Status:** VERIFIED
- **Intent:** The comment repeated beside all three branches names the denial case, and the comment at `src/Billing/BillingUtilities.php:L1613` names the process-file column as the denial reason store. Read as evidence of intent, the branch exists to record the denial and its reason without advancing the claim's billing state. INFERRED (confidence: Medium): the omission reads as a deliberate refusal to advance rather than as an oversight, because the general branch it sits beside writes both columns explicitly and the denial branch was written as a peer of it rather than as a case inside it. Basis: the two branches are adjacent at `src/Billing/BillingUtilities.php:L1586-L1600` and no comment states why the billed columns are skipped, so the reason for the omission is not recorded anywhere. The consequence is a separate matter from the intent, because leaving the columns alone leaves them holding a value the denial contradicts; that consequence is traced through to what an operator sees in [claim-lifecycle.md](claim-lifecycle.md) and its implications are carried in [defect-candidates.md](defect-candidates.md).
- **Novel:** yes, with one existing-source touchpoint on the same topic. `Documentation/help_files/sl_eob_help.php:L156` tells the operator that denied claims are not handled automatically and will appear in red on the posting report, so the *existence* of special denial handling is visible to a reader of the help. What is new here is the rule itself and everything about how it is recorded: that the internal status value is 7, that it is the only status carrying its own branch at all three steps of the claim update, that the value is written into the queue's process column, that the denial reason is stored in the claim's process-file column, and that the billed flag and billed date are left untouched rather than cleared. Absent from Readme_edihistory.html, DEVELOPER_GUIDE.md and cms_1500_help.php.
...
That is `src/Billing/BillingProcessor/BillingClaim.php:L137-L138`. One query per claim, executed inside the constructor, whose result is guarded for emptiness and retained as the claim's target at `src/Billing/BillingProcessor/BillingClaim.php:L139-L142`.
...
That is `src/Billing/BillingProcessor/BillingProcessor.php:L113-L116`, with the screen message at `src/Billing/BillingProcessor/BillingProcessor.php:L115` omitted from the excerpt. The `continue` drops the claim from the collection being assembled, and because the condition is conjunctive the same claim survives a run whose electronic flag is unset.
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** The rebuild joins every modifier recorded for that code with colons, so a charge carrying two modifiers is matched by a three-part key and a charge carrying one by a two-part key. The loop that builds it iterates the whole code list and overwrites the key on each match, so if a code appears more than once the last occurrence wins. A failure to match here does not reach the charge sanity check registered as [BR-A9](#br-a9-a-charge-that-disagrees-with-the-invoice-by-one-cent-blocks-the-posting), because that check sits inside the opposite branch: an empty previous charge sends control to the unmatched-service branch at `interface/billing/sl_eob_process.php:L495-L535` instead, and what happens there is decided by a site-level switch rather than by any comparison of amounts. With the switch off, `interface/billing/sl_eob_process.php:L504` raises the error flag and the line is reported as merely returned by the payer, so nothing is posted for it. With the switch on, no error is raised and the payer's own reported charge is inserted into the charge table at `interface/billing/sl_eob_process.php:L507-L521` and added to the running invoice total; that switch is registered as [BR-I3](#br-i3-one-switch-turns-a-payer-reported-unknown-code-into-a-charge).
...
That is `interface/billing/sl_eob_process.php:L462-L464`, reached only under the condition at `interface/billing/sl_eob_process.php:L461`, which requires that no charge matched the key, that the payer sent no modifier, and that the bare code appears in the list built from the invoice's own charge keys at `interface/billing/sl_eob_process.php:L436-L443`. The rebuilt key joins every modifier recorded for that code with colons, and the loop assigns on each match rather than stopping at the first.
...
That is `src/Billing/Claim.php:L715-L717`. The test is an identity comparison against the empty string on an untrimmed value, so a partner row holding a single space counts as configured.
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** This function decides which payer a claim is billed to and which payer a remittance is credited to, and the ordering is what settles a contest between matching rows. Because a null sorts lowest, `ORDER BY date DESC` places an undated row after every dated one, so an undated row is selected only when no dated row matches the service date at all. An undated row is therefore a fallback rather than an override, and the exposure runs the opposite way to what that ordering might suggest: adding a start date to an existing row promotes it above the undated one and can change which payer future claims go to, with no message when it does. Where several undated rows match, the query offers no further tiebreak and the row returned is whichever the server yields first. The same function returns zero for a level outside one to three, at `src/Billing/SLEOB.php:L252-L254`, but that guard is not what produces the tertiary behaviour in [BR-E2](#br-e2-the-service-date-is-the-first-ten-characters-of-the-encounter-timestamp), because the advance on that path never asks for a level outside the range.
...
- **Statement:** When the subsystem needs a date of service in order to find the next payer, it takes the first ten characters of the encounter's stored timestamp rather than converting it. The next insurance level is then derived by advancing the encounter's last-billed watermark, and the advance stops at three rather than passing beyond it: a watermark already at three is not incremented, so the level looked up is the same tertiary level that was just billed.
- **Evidence:** `src/Billing/SLEOB.php:L284`, with the level advance at `src/Billing/SLEOB.php:L285-L288` and the payer lookup at `src/Billing/SLEOB.php:L290`. The two outcomes of that lookup are the queue branch at `src/Billing/SLEOB.php:L292-L296` and the reopen branch at `src/Billing/SLEOB.php:L297-L302`.
- **Status:** VERIFIED
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** The advance condition is a mixture of comparisons that lets the level reach three but never four. PHP evaluates the conjunction before the disjunction, so at a watermark of exactly three both alternatives are false and the increment at `src/Billing/SLEOB.php:L287` is skipped; the lookup is then made for level three again, returns the same tertiary payer that was just paid, and the claim is queued straight back to that payer at `src/Billing/SLEOB.php:L295`. The reopen branch at `src/Billing/SLEOB.php:L297-L302` is not the tertiary outcome. It is the no-coverage-on-file outcome, reached whenever the level the advance settled on has no matching coverage row, which includes a patient who has a secondary but no tertiary. Neither branch reports itself: the routine is declared void at `src/Billing/SLEOB.php:L271` and emits nothing, while the caller prints one fixed sentence naming secondary paper billing at `interface/billing/sl_eob_process.php:L721-L725` on every non-crossover path. A tertiary requeue and an unrouted reopen are therefore indistinguishable on screen from an ordinary advance to secondary. The consequences are carried in [defect-candidates.md](defect-candidates.md), and both paths are traced with their operator-visible symptoms in [claim-lifecycle.md](claim-lifecycle.md).
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** VERIFIED: the converter is invoked seven times from six expressions, all of them UB-04 form fields on an institutional claim - the statement period at `src/Billing/X125010837I.php:L387` and `src/Billing/X125010837I.php:L388`, the admission date at `src/Billing/X125010837I.php:L395`, the two ends of an occurrence span at `src/Billing/X125010837I.php:L519`, which calls it twice within one expression, the occurrence date at `src/Billing/X125010837I.php:L550` and the value-information date at `src/Billing/X125010837I.php:L582`. Dates of birth are not among them: the subscriber's birth date is emitted through a different accessor, at `src/Billing/X125010837I.php:L265`, and never reaches this function. The seven invocations divide into two kinds: three are anchored to the encounter itself, being the statement period and the admission date, and four are anchored to a reported event, being the two ends of the occurrence span, the occurrence date and the value-information date. INFERRED (confidence: High): the hardcoded century is effectively always right for the three encounter-anchored fields and is a live exposure on the four event-anchored ones, because an encounter date before 2000 would mean a claim filed decades late whereas an occurrence, occurrence-span or value-information field records when a reported event happened rather than when the encounter did. Basis: the UB-04 field positions the four event-anchored calls read are the occurrence, occurrence-span and value-code groups, and no code in the generator constrains how far in the past those may fall. The exposure therefore covers the value-information field as well as the occurrence fields, rather than the occurrence fields alone. Changing the literal to a computed century would change every date this function produces, which is why the two tests that pin its output exist; they run under the isolated configuration described in [upgrade-risk-map.md](upgrade-risk-map.md).
...
That is `src/Billing/X125010837I.php:L19-L21`. The six-character date is the method's only parameter, so no argument and no lookup supplies a century; the same three lines are read for a different consequence by [BR-E4](#br-e4-an-empty-institutional-date-becomes-the-two-character-string-20).
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** INFERRED (confidence: High): a payer or clearinghouse validating that element against the implementation guide would reject the claim, and the rejection would arrive as an acknowledgement rather than at generation time, so the operator would learn of it from the clearinghouse rather than from the screen. Basis: the same converter produces eight characters for any input it can read, so two characters is a malformed date rather than an unusual one; but no payer-side validator exists anywhere in this repository, so the outcome is reasoned about rather than observed. VERIFIED: because a test pins this output at `tests/Tests/Isolated/Billing/X125010837IDateTest.php:L38`, any change that makes the converter reject an empty input will fail that test, which is the correct place for the decision to be made.
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** INFERRED (confidence: Medium): a payer or clearinghouse in a different timezone would read these as its own local time, so a batch transmitted late in the evening could be dated a day earlier or later than the payer's own record of receiving it, which would matter wherever a filing deadline is counted in days. Basis: the values carry no offset and no zone marker, so a reader has nothing to interpret them against; but which zone any given payer assumes is not observable from this repository, and the confidence is Medium rather than High because a payer that treats the value as its sender's local time would see no discrepancy at all. VERIFIED: the values are produced from one timestamp captured once per file, so every claim in a batch carries the same pair and any discrepancy is uniform rather than per claim. VERIFIED: the zone comes from a single site-level configuration value rather than from anything in the billing code, so changing that value changes the timestamp on every subsequent envelope, and on every database timestamp written in the same request, with no change to any file in this subsystem.
...
- **Statement:** When a payer other than the primary reports an adjustment, the amount is not posted. A ledger line is still written so that the note survives, and the amount on that line is the literal zero. Within the persisted row the payer's own figure survives only inside the note text, as a formatted number. On the screen it is rendered twice more, once as a message line and once, negated, in the detail row, so what an operator reads and what the ledger stores differ for this branch.
- **Evidence:** `interface/billing/sl_eob_process.php:L612-L631`. The note is built at `interface/billing/sl_eob_process.php:L616`, the payer's amount is appended to the note text at `interface/billing/sl_eob_process.php:L619`, and the ledger line is written by the call at `interface/billing/sl_eob_process.php:L622-L631` whose amount argument is the literal zero at `interface/billing/sl_eob_process.php:L626`. The two on-screen renderings of the payer's figure are the message line at `interface/billing/sl_eob_process.php:L634-L635` and the detail row at `interface/billing/sl_eob_process.php:L656-L665`, which negates it at `interface/billing/sl_eob_process.php:L664`.
- **Status:** VERIFIED
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** VERIFIED: an undelivered claim batch carries the same stored status as a delivered one, so any query or screen that filters on that status returns it among the delivered batches, and no further transmission is attempted for it by this code. The failure messages are preserved on the record, so the evidence exists in a column that status-based filtering does not read. VERIFIED: the failure text itself is not lost, and it is discoverable, but only by hand and only behind a row that presents itself as a success. The messages recorded at `src/Billing/BillingProcessor/X12RemoteTracker.php:L114-L115` are persisted by the update at `src/Billing/BillingProcessor/X12RemoteTracker.php:L116` and survive the status overwrite, because the overwrite at `src/Billing/BillingProcessor/X12RemoteTracker.php:L120-L121` replaces only the status field of the same record. They are returned to the browser at `library/ajax/billing_tracker_ajax.php:L50` and rendered as alert blocks by the row-detail formatter at `interface/billing/billing_tracker.php:L143-L147`, which runs only when an operator clicks to expand that row, at `interface/billing/billing_tracker.php:L174-L187`. INFERRED (confidence: High): in practice the affected claims are therefore not resubmitted until something outside this subsystem notices, which can be as late as the payer's filing deadline. Basis: the status column is the only field the transport path re-reads, nothing schedules a retry, nothing raises the failure proactively, and the row carries no failure marking to prompt the expansion that would reveal the messages. This entry is a rule about how transport outcome is recorded; the defect is carried in [defect-candidates.md](defect-candidates.md), and the transport stage is described in [claim-lifecycle.md](claim-lifecycle.md).
...
- **Statement:** While assembling a batch, the writer rebuilds the interchange header and then checks that the result is exactly 105 characters. If it is not, the request is terminated immediately. The same happens if the first segment of a claim is not an interchange header. Both checks sit behind a test that the batch buffer is still empty, so in the ordinary case they can fire only on the first claim of the run that contributes a segment. Neither case is an exception that a caller could handle.
- **Evidence:** The length check and termination are at `src/Billing/BillingProcessor/BillingClaimBatch.php:L219-L222`, inside the empty-buffer test at `src/Billing/BillingProcessor/BillingClaimBatch.php:L215`; the leading-segment check and termination are the `elseif` arm of that same test, at `src/Billing/BillingProcessor/BillingClaimBatch.php:L225-L227`. The batch file has one writer, `write_batch_file()` at `src/Billing/BillingProcessor/BillingClaimBatch.php:L153-L166`, which the generator calls from its completion method at `src/Billing/BillingProcessor/Tasks/GeneratorX12.php:L202` once the whole batch has been assembled.
- **Status:** VERIFIED
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** The termination ends the request part-way through a run over many claims, and the operator sees a truncated page ending in the message. VERIFIED: no batch file is produced at all, not an empty one and not a partial one, because the only writer is reached from the completion method at `src/Billing/BillingProcessor/Tasks/GeneratorX12.php:L202` and the request never arrives there; the name that writer would have used is stamped to the second at `src/Billing/BillingProcessor/BillingClaimBatch.php:L64`, so no later run collides with it either. VERIFIED: no claim in the run is queued for transmission, because the queue write sits in that same unreached method, at `src/Billing/BillingProcessor/BillingClaimBatch.php:L177-L184`. What does survive is a database residue on the claim being appended when the abort fired. VERIFIED: the abort happens inside `updateBatchFile()` at `src/Billing/BillingProcessor/Tasks/GeneratorX12.php:L165`, which sits between the generator's two updater calls, so the first call has already marked the claim billed and inserted a new claim version, at `src/Billing/BillingProcessor/Tasks/GeneratorX12.php:L151-L162`, while the second call, the one that would record the batch filename, at `src/Billing/BillingProcessor/Tasks/GeneratorX12.php:L168`, never runs. Nothing reverses the marking, because the request ends inside `die()`. INFERRED (confidence: High): the amount consequence is that the claim reads as billed while no transmission of it exists anywhere, so no charge on it is adjudicated until an operator notices and repeats the run. Basis: the marking is committed before the abort, the queue write it precedes is never reached, and nothing records that the run was attempted. The full row-state account, including the stale filename a previously billed claim keeps on its charge rows, is in [claim-lifecycle.md](claim-lifecycle.md). The width the check enforces is what the header rebuild in [BR-E6](#br-e6-the-professional-interchange-date-is-a-placeholder-the-batch-replaces) depends on, so the two are one mechanism.
...
That is `src/Billing/BillingProcessor/BillingClaimBatch.php:L219-L221`. The length is measured on the accumulated batch content rather than on the segment alone, and the branch ends the request outright instead of returning a failure to its caller. Both this check and the leading-segment check below it are reached only while that accumulated content is still empty, which is what confines them to the first contributing claim.
...
That is `src/Billing/BillingProcessor/BillingClaimBatch.php:L177-L180`, with the partner element at `src/Billing/BillingProcessor/BillingClaimBatch.php:L179` omitted from the excerpt. The filename queued is the one batch file the run produced, so every distinct partner in the batch receives a queue row naming the same file; the waiting status and the serialised claims payload complete the row at `src/Billing/BillingProcessor/BillingClaimBatch.php:L181-L182`.
...
- **Status:** VERIFIED
- **Intent:** The comment block above the segment, at `src/Billing/X125010837P.php:L778-L784`, records that medical attachments are not implemented and that the three code values are hardcoded. Read as evidence of intent, the segment was added as a placeholder for a feature that was never built, which is consistent with the trading-partner attachment endpoint column having no operational consumer - one of the five endpoint and credential columns catalogued as dead in [transactions.md](transactions.md), where dead means that no transport or client acts on the stored value.
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** The transaction reports one fewer segment than it contains for every employment-related claim, and the batch copies that count through unchanged per [BR-H7](#br-h7-the-segment-count-is-copied-from-the-generator-into-the-batch-unchanged), so the discrepancy reaches the payer. Nothing in the generation path detects it, so if the payer rejects the transaction the rejection cannot surface at generation time; it can only arrive on a later acknowledgement, and an acknowledgement becomes visible here only once an operator uploads the returned file by hand, as [claim-lifecycle.md](claim-lifecycle.md) traces. Until that happens the claim reads as sent and the charge stays open. Whether any given payer rejects on the count at all is not determinable from this repository, since no acknowledgement fixture or partner-specific rule in the codebase records it. Separately, the claim announces an attachment that no transport exists to send, because the trading-partner column that would hold the endpoint to send it to has no operational transport or client consumer. VERIFIED: that column is not unread, and the distinction matters to anyone auditing an installation. It is rendered as an editable text field on the trading-partner edit screen, at `templates/x12_partners/general_edit.html:L214-L219`, alongside the other four columns of the same unbuilt integration, which occupy the blocks from `templates/x12_partners/general_edit.html:L179` to `templates/x12_partners/general_edit.html:L212`; a posted value is assigned by the controller base's generic setter walk, at `controllers/C_X12Partner.class.php:L76` reaching `library/classes/Controller.class.php:L104-L112`, and persisted at `controllers/C_X12Partner.class.php:L85`. So an operator can enter an endpoint, save it, and see it again on reload, and nothing will ever send an attachment to it. Both consequences are registered in [defect-candidates.md](defect-candidates.md).
...
- **Statement:** The modern remittance parser recognises a fixed vocabulary of segments and returns an error naming any segment outside it. Returning that error ends the parse at the segment that caused it. The Medicare inpatient adjudication segment is outside the vocabulary, so a remittance carrying one is read up to the point the segment appears and no further. The outcome is position-dependent rather than all-or-nothing, and it is position-dependent in what the parser *emits* rather than in what accounts receivable *records*: every claim whose own record was already flushed before that point has been handed to the caller's callback, while the claim being read when the segment arrives, together with every claim after it in the file, is never handed over at all. What each handed-over claim then does to accounts receivable is decided by the callback, not by the parser, and the posting callback writes only for a claim that satisfies every one of its own guards.
- **Evidence:** The rejection is the final branch of the parser's segment dispatch, at `src/Billing/ParseERA.php:L467-L468`. What is emitted before it is decided by the four points at which the parser hands a completed record to its caller: the transaction header at `src/Billing/ParseERA.php:L144`, the service-line loop marker at `src/Billing/ParseERA.php:L229`, the claim payment header at `src/Billing/ParseERA.php:L241`, the transaction trailer at `src/Billing/ParseERA.php:L442`, and the callback invocation itself, which is the single statement `$cb($out);` at `src/Billing/ParseERA.php:L81`. VERIFIED: that statement is the whole of the parser's contribution, so the parser establishes delivery and nothing more. The posting callback on the operator path is `eob_process_era_callback()`, declared at `interface/billing/sl_eob_process.php:L297` and passed to the parser by name at `interface/billing/sl_eob_process.php:L851`, and it writes nothing unless the operator selected the check, at `interface/billing/sl_eob_process.php:L306`; unless the claim resolves to an encounter this practice holds, since a missing encounter row zeroes the patient and encounter numbers at `interface/billing/sl_eob_process.php:L336-L338` and leaves the invoice-error flag raised, reported at `interface/billing/sl_eob_process.php:L361-L366`; and unless the claim status permits posting, since a denial raises the same flag at `interface/billing/sl_eob_process.php:L379-L381` and a payment reversal at `interface/billing/sl_eob_process.php:L404-L405`. That flag becomes the error flag at `interface/billing/sl_eob_process.php:L433`, and every write is then guarded on it together with the dry-run flag, at `interface/billing/sl_eob_process.php:L507`, `interface/billing/sl_eob_process.php:L559`, `interface/billing/sl_eob_process.php:L621` and `interface/billing/sl_eob_process.php:L642`. The legacy renderer recognises the segment at `library/edihistory/edih_835_html.php:L531` and renders eight of its elements at `library/edihistory/edih_835_html.php:L532-L545`. A search of `src/Billing/ParseERA.php` for that segment identifier returns nothing.
- **Status:** VERIFIED
...
- **Blast radius:** The operator sees the file listed and readable in the history browser, and sees the posting screen stop with a message naming a segment identifier and no indication that the segment is unsupported rather than malformed. Because the stop is positional, the screen's state at that moment is a partial run rather than a rejected file, and nothing on it distinguishes the claims that were committed from the claims that were not. A file whose fortieth claim carries the segment hands the thirty-nine before it to the posting callback and never hands over the fortieth or anything after it; how many of those thirty-nine actually reached accounts receivable depends on the callback's guards cited above, so the posted count is at most thirty-nine and may be fewer, and it is zero in a dry run. The count is not recoverable from the screen. The consequence of the positional stop, and the defect entry for it, are carried in [defect-candidates.md](defect-candidates.md).
...
- **Statement:** A claim is identified throughout this subsystem by the patient number and the encounter number joined with a hyphen, patient first. The identifier is not a database key. The claim record is keyed by a three-column primary key of patient, encounter and version, so the two numbers the identifier carries do not on their own address a single row: they address the whole version history of one claim.
- **Evidence:** The convention is documented at `src/Billing/BillingProcessor/BillingClaim.php:L31-L38`, parsed at `src/Billing/BillingProcessor/BillingClaim.php:L114-L117` and emitted onto the claim at `src/Billing/X125010837P.php:L675`. The three-column key is declared at `sql/database.sql:L392`, within the table at `sql/database.sql:L378-L392`, whose version column carries its own comment at `sql/database.sql:L381`.
- **Status:** VERIFIED
...
That is `sql/database.sql:L10193-L10195`, the three columns that say what a ledger line is about, ending with the one this rule concerns. The same value is the unknown sentinel in the claim model, declared at `src/Billing/BillingProcessor/BillingClaim.php:L79`.

### BR-G4 A ledger line is either a payment or an adjustment, by comment alone

- **Statement:** A ledger line holds a payment amount and an adjustment amount, and at least one of the two is intended to be zero. The rule is recorded only in a column comment. Nothing in the schema enforces it, and the two posting helpers each hardcode the other field to zero to maintain it. A row carrying zero in both is not a violation of the rule and is written deliberately: the non-primary note path posts a zero adjustment, and the adjustment helper it goes through sets the payment to zero as well.
- **Evidence:** The comment is at `sql/database.sql:L10200`. The payment helper writes the literal zero adjustment at `src/Billing/SLEOB.php:L156` and the adjustment helper writes the literal zero payment at `src/Billing/SLEOB.php:L244`. The both-zero row comes from the non-primary note path at `interface/billing/sl_eob_process.php:L621-L631`, whose amount argument is the literal zero at `interface/billing/sl_eob_process.php:L626`, arriving at that same zero payment at `src/Billing/SLEOB.php:L244`; the branch itself is registered as [BR-F1](#br-f1-non-primary-insurance-adjustments-are-recorded-as-notes-worth-zero).
- **Status:** VERIFIED
- **Intent:** The column comment states the rule directly, at `sql/database.sql:L10200`: either the payment or the adjustment will always be zero. Read as evidence of intent, that is a declared rule about the shape of a row rather than an accident of usage. Its wording is permissive rather than exclusive: it says one of the two will always be zero, not that exactly one will, which is what leaves room for the deliberate both-zero row. INFERRED (confidence: Medium): the shape was chosen so that one table could serve as both a cash ledger and an adjustment ledger without a discriminator column, the zero amount standing in for one. Basis: the table has no type or kind column, both amount columns are non-nullable with a zero default, and the comment records the rule without recording why it was preferred to an explicit discriminator.
...
- **Blast radius:** The rule is maintained by two call sites and asserted by none, so any third writer that set both fields to non-zero values would produce a row that every reader misinterprets, and nothing would reject it. The database declares no check constraints anywhere, verified by searching `sql/database.sql` across all 283 table definitions, so this is representative rather than exceptional: the invariants in this schema live in comments. A reader who strengthens the rule to exactly one zero also misreads the ledger, because the both-zero note rows are legitimate and are numerous wherever secondary remittances are posted. Both helpers pass the zero as a string rather than a number, which the fixed-point column coerces.
...
- **Statement:** The claim model carries a single cleaner that uppercases a value and then removes every character outside a fixed permitted set. It is invoked one call site at a time rather than standing as a boundary that the outbound path passes through, so a value that reaches an outbound claim without a call to it is not filtered at all, and every trading-partner value in the interchange header and the functional group header is of that second kind. The permitted set is letters, digits, space and fifteen punctuation characters, which are, in the order the character class lists them: `!` `"` `&` `'` `(` `)` `+` `,` `-` `.` `/` `;` `?` `=` `@`. Anything else, including any accented letter, is deleted rather than replaced.
- **Evidence:** `src/Billing/Claim.php:L217-L220`, a single expression at `src/Billing/Claim.php:L219`. The docblock at `src/Billing/Claim.php:L212` names the character set the filter enforces and the page of the implementation guide that defines it. That the cleaner is a call rather than a boundary is visible in the same file: the eight interchange-header accessors at `src/Billing/Claim.php:L674-L710` return their stored trading-partner values unaltered, the sender and receiver identifiers at `src/Billing/Claim.php:L624-L631` and `src/Billing/Claim.php:L664-L672` are padded to fifteen characters without being filtered, and the two functional-group accessors at `src/Billing/Claim.php:L713-L721` and `src/Billing/Claim.php:L634-L651` return their stored values or fall back to another stored value. Both generators emit those accessors straight into the interchange header and the functional group header, at `src/Billing/X125010837P.php:L60-L88` and `src/Billing/X125010837I.php:L44-L74`. One accessor in that same region does call the cleaner, at `src/Billing/Claim.php:L660`, which is what makes the per-call-site pattern legible rather than inferred.
- **Status:** VERIFIED
- **Intent:** The docblock states it, at `src/Billing/Claim.php:L212`: the function enforces the format's basic character set. Read as evidence of intent, that makes the cleaner a format-conformance step rather than a general-purpose input sanitiser, and the distinction cuts both ways rather than only downwards. VERIFIED: on the values it does process it has one structural effect, because the permitted set at `src/Billing/Claim.php:L219` excludes all three delimiters these generators use - the element separator `*` and the segment terminator `~`, both of which appear as literals throughout the emission at `src/Billing/X125010837P.php:L60-L88`, and the component separator declared as the last element of the interchange header at `src/Billing/X125010837P.php:L76` and `src/Billing/X125010837I.php:L62` - so a delimiter character arriving inside a cleaned value is deleted rather than emitted into the segment stream. VERIFIED: it is nonetheless not a general input-validation control, on three counts visible in its own definition and call sites - it acts only where a caller calls it, it filters characters without validating that the remaining value means anything, and it is declared on the claim model for the X12 output path rather than on any inbound path.
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** Deleting rather than replacing means a name written with an accented letter loses that letter entirely rather than gaining a substitute, so a patient's name can reach a payer misspelled in a way that fails an eligibility match. Because the filter runs after the uppercase, adding a lowercase character to the permitted set would have no effect. The cleaner is also applied to monetary values on the way into the charge accumulation registered as [BR-A4](#br-a4-charge-totals-are-accumulated-as-floats-and-printed-at-two-decimals), where the uppercase is inert but the character filter is not. The values the cleaner is never called on are outside this rule entirely: the envelope accessors named in the evidence are emitted as stored, so nothing in this entry describes what reaches the wire through them.
...
That is `src/Billing/Claim.php:L217-L219`; the closing brace on the line below ends the method, so its body is that one expression, with no validation and no error return, which is why calling it is the only way a value gets filtered. The docblock immediately above it, at `src/Billing/Claim.php:L211-L216`, is what names the character set and the page of the implementation guide that defines it.
...
That is `src/Billing/BillingUtilities.php:L1754-L1756`. The accumulator starts at minus one at `src/Billing/BillingUtilities.php:L1736`, so a single unbilled row forces it to zero and no later row can lift it, because the other arm, at `src/Billing/BillingUtilities.php:L1756-L1760`, raises the value only while it is still negative.
...
That is `src/PaymentProcessing/Recorder.php:L45-L46`, the query having no ordering clause and no limit; the column name and the bound reference complete the call at `src/PaymentProcessing/Recorder.php:L47-L49`. The column is declared `NOT NULL DEFAULT ''` with no unique index at `sql/database.sql:L10163`, so an unset reference is the empty string and matches every other unset reference, and the query returns whichever such row the database yields first.
...
That is `src/Billing/BillingUtilities.php:L1678-L1680`. The transaction that encloses it opens at `src/Billing/BillingUtilities.php:L1677`, the aggregate is read with no `FOR UPDATE` clause and no retry around it, and the failure mode is registered in [defect-candidates.md](defect-candidates.md).
...
That is `src/Billing/BillingUtilities.php:L1698-L1702`, with the two identifier columns at `src/Billing/BillingUtilities.php:L1699-L1700` omitted from the excerpt. Five columns are named in all, the version among them is bound from the computed increment, and every other column of the row takes its schema default.

### BR-H4 Interchange and group control numbers are two separate draws on one sequence

- **Statement:** A batch's interchange control number and its functional group control number are drawn separately from the same site-wide counter. The interchange number is zero-padded to nine characters and the group number is not padded at all. The two draws are independent calls with no transaction spanning them, so within one envelope the two numbers are normally adjacent values of the same counter rendered two different ways, but the adjacency follows from nothing else drawing in between rather than from any guarantee the code makes.
...
- **Blast radius:** The `(9)` in the column declaration is a display hint rather than a constraint, so the counter is an ordinary 32-bit unsigned integer holding values up to 4294967295 with no key on it. Any value above 999999999 renders as ten characters, at which point the pad becomes a no-op and the interchange control number is emitted one character too wide. That widens the interchange header past the fixed length the batch rebuild depends on, and it is the length check registered as [BR-F4](#br-f4-a-malformed-interchange-header-terminates-the-whole-batch-run) that would catch it, by terminating the run. Because the two draws are unsynchronised, two batch runs overlapping in time interleave their values, so an envelope can carry an interchange number and a group number that are not adjacent; nothing reads the pair back afterwards, so the interleaving leaves no trace. Separately, every draw consumes a value even when the resulting file is never transmitted, and the attachment segment in [BR-F6](#br-f6-the-attachment-segment-is-emitted-without-being-counted) draws one per employment-related claim.
...
Those are the two returns, at `src/Billing/BillingProcessor/BillingClaimBatchControlNumber.php:L22-L30`, with the two method signatures, their braces and the blank line between the methods omitted from the excerpt. Both returns call the same generator, so one batch consumes two values from one sequence, and only the interchange value is padded to nine characters.
...
That is `src/Billing/BillingProcessor/BillingClaimBatch.php:L63-L66`, with the batch filename and directory assignments at `src/Billing/BillingProcessor/BillingClaimBatch.php:L64-L65` omitted from the excerpt. Both ternaries test the action string of the first claim in the batch, so the substitution is decided once for the whole batch rather than per claim.
...
- **Blast radius:** The rewrite finds the marker by substring search and replaces six characters at that offset, so if a claim ever contains the same six characters earlier in its transaction header the wrong span is replaced. A missing marker is worse than a misplaced one, and it does not leave the reference alone. VERIFIED: `strpos()` returns `false` when it finds nothing, the file declares no strict types, and the offset parameter is typed as an integer, so the `false` arrives at `substr_replace()` as zero and the replacement lands at the start of the segment instead of at the marker. VERIFIED: the segment reaching that call always begins `BHT*00`, because the generator composes it in that order at `src/Billing/X125010837P.php:L108-L111` and the caller splits on the segment terminator and newline at `src/Billing/BillingProcessor/Tasks/GeneratorX12.php:L68-L69`, leaving no leading characters. Those first six characters are what get overwritten with three, so the segment loses its own identifier rather than keeping an unrewritten reference. Changing the literal in either generator without changing the batch's search string therefore corrupts the transaction header of every claim in the batch, and the two generators plant the same literal independently. This is the rule where an existing document and the current code agree most directly, and the defect entry is carried in [defect-candidates.md](defect-candidates.md).
...
- **Statement:** The modern file scanner decides whether a file is a usable interchange by returning a sentinel string, and each of four later validity questions is answered by searching that string for one character. The string is not built up a character at a time. It is assigned outright as one of four fixed literals of two, three, four and five characters, and each assignment overwrites whatever the previous one wrote rather than extending it.
- **Evidence:** The four assignments are `src/Billing/EdiHistory/X12File.php:L324`, `src/Billing/EdiHistory/X12File.php:L327`, `src/Billing/EdiHistory/X12File.php:L331` and `src/Billing/EdiHistory/X12File.php:L335`, together spanning `src/Billing/EdiHistory/X12File.php:L324-L336`; the initial empty value is at `src/Billing/EdiHistory/X12File.php:L292`. The functional-group and transaction-set tests are two independent `if` statements, at `src/Billing/EdiHistory/X12File.php:L330` and `src/Billing/EdiHistory/X12File.php:L334`, rather than a chained pair. The four questions are answered at `src/Billing/EdiHistory/X12File.php:L114-L117`.
- **Status:** VERIFIED
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** Because each literal is written unconditionally rather than derived from what was found, two of the four answers can be true when the thing they name is absent. VERIFIED: the transaction-set test at `src/Billing/EdiHistory/X12File.php:L334` writes the five-character literal whether or not the functional-group test above it succeeded, so a file carrying an interchange header and a transaction set header but no functional group header is still reported as having a functional group. VERIFIED: the two-character literal is assigned at `src/Billing/EdiHistory/X12File.php:L324` before any envelope segment is looked for, so any file clearing the mime and character screens is reported valid even with no X12 content in it at all. The four questions are answered by taking a string position and casting it to a boolean, which is false for position zero. That is correct only because the sentinel always begins with a character none of the four questions asks about, so no searched character can ever sit at position zero, and changing that leading character silently inverts one of the four answers. The same position-zero hazard sits inside the scanner's own two tests, which are truth-tested rather than compared against false; there it is harmless only because a file that reaches them begins with an interchange header, so neither searched string can occur at offset zero. The scanner also refuses a file outright when it contains a character outside the printable range or matches one of several script-like patterns, at `src/Billing/EdiHistory/X12File.php:L318-L322`, returning the sentinel unchanged from the initial empty value at `src/Billing/EdiHistory/X12File.php:L292` - the one case in which all four answers are false. These consequences are carried in [defect-candidates.md](defect-candidates.md).
...
That is `src/Billing/EdiHistory/X12File.php:L324-L327`, with the comment on the first line preserved because it is the only statement of what the string means. Both lines assign a whole literal, which is what makes the later assignments overwrite rather than extend.

## Group I Site Global Gated Rules

Six rules. A site global is an administrator-facing setting stored per site. Five of the six entries below are switches whose position changes a dollar amount, a payer routing decision, a payer identifier, or whether anything is transmitted at all, and the remaining one is a schema default that behaves like a switch. Each entry names the declaration, its shipped default and its consumers, because a rule that only fires under a setting is not documented until the setting is named.
...
That is `library/globals.inc.php:L1297-L1300`, with the display label at `library/globals.inc.php:L1298` omitted from the excerpt. The default is the string `1` on a boolean-typed setting, so the behaviour is active on a site that has never opened the setting.
...
- **Statement:** When a remittance reports a service the practice has no charge for, the default behaviour is to treat it as an error. The flag it raises is the same one the rest of the loop reads, so it suppresses posting for that service line and for every service line after it in the same claim, while leaving in place whatever that claim posted before it. One switch changes that to inserting a new charge into the charge queue, built from the payer's own code and amount, with a description naming the insurance level and the production date, and the loop continues normally.
- **Evidence:** The switch is `add_unmatched_code_from_ins_co_era_to_billing`, declared at `library/globals.inc.php:L1564-L1569`, with the default at `library/globals.inc.php:L1567`. The consumer is `interface/billing/sl_eob_process.php:L501`, whose two arms are `interface/billing/sl_eob_process.php:L502` and `interface/billing/sl_eob_process.php:L504-L505`, and the insert follows at `interface/billing/sl_eob_process.php:L507-L521`. The flag raised in the second arm, at `interface/billing/sl_eob_process.php:L504`, is the flag the enclosing loop reads, and that loop runs from `interface/billing/sl_eob_process.php:L449` to `interface/billing/sl_eob_process.php:L668` with no transaction around it.
- **Status:** VERIFIED
...
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** With the switch on, a payer can add a charge to a practice's charge queue by reporting a service the practice never billed, and the amount on that charge is the payer's reported charge. The insert runs through the helper registered as [BR-F8](#br-f8-the-remittance-charge-helper-declares-a-dry-run-flag-and-never-reads-it), and it passes a deposit identifier of zero at `interface/billing/sl_eob_process.php:L511` that the helper discards. It runs only when neither the error flag nor the dry-run flag is set, because the guard at `interface/billing/sl_eob_process.php:L507` encloses both the call and the invoice-total update, so a preview neither creates the charge nor advances the total. The charge is inserted as unauthorised, so it does not immediately become billable, but it is present and the invoice total is advanced by it at `interface/billing/sl_eob_process.php:L520`. VERIFIED: the switch itself only chooses between the two descriptions at `interface/billing/sl_eob_process.php:L502` and `interface/billing/sl_eob_process.php:L504-L505`; it is the error flag set in the second arm that then suppresses the insert. That flag reaches further than the insert, because it is the loop-scoped flag whose outcome is registered in [BR-A9](#br-a9-a-charge-that-disagrees-with-the-invoice-by-one-cent-blocks-the-posting): with the switch off, one unmatched code stops the remaining service lines of that claim from posting and leaves the earlier ones in place, so turning the switch on changes not only whether a charge is created but how much of the claim posts at all.
...
That is `interface/billing/sl_eob_process.php:L501-L503`. The branches differ in one further respect: only the second, at `interface/billing/sl_eob_process.php:L504-L505`, sets the error flag, and that flag is what the guard at `interface/billing/sl_eob_process.php:L507` tests before anything is written.
...
That is `src/Billing/BillingProcessor/BillingClaimBatch.php:L170-L172`. The switch is read on every batch and gates the queue write that follows the guard, and it is the second operand of a conjunction whose first operand requires the local file write to have returned the boolean true.
...
That is `sql/database.sql:L10039-L10040`. The declared default on the first of the two is the production indicator, so a partner row inserted without naming that column transmits live; the empty default on the line below it is the value that the application sender fallback registered as [BR-D7](#br-d7-the-application-sender-code-falls-back-to-the-interchange-sender-identifier) tests for, so the same insert also inherits that fallback.

### BR-I6 One switch decides what the interface offers and which payer identifier the request carries

- **Statement:** A single site switch, shipped off, does two unrelated things. It decides whether the eligibility actions are offered on screen - and on one of the two entry points it is also tested on the server before a request is made, while on the other it is not, so with the switch off the feature is withdrawn from view but only one of the two paths is actually prevented from transmitting. And it decides which of two `insurance_companies` columns supplies the payer identifier written into the name segment of the 270. With the switch on, the request carries the dedicated eligibility identifier; with it off, the request carries the claims identifier instead. The switch is read at the moment the segment is built, not at configuration time, so turning it on or off changes the identifier on the wire for every subsequent request without any partner or payer row being edited.
- **Evidence:** The switch is `enable_eligibility_requests`, declared under the insurance group at `library/globals.inc.php:L2060-L2065`, typed boolean at `library/globals.inc.php:L2062` and defaulting to off at `library/globals.inc.php:L2063`. The identifier substitution is a single ternary at `src/Billing/EDI270.php:L167`, whose result is assigned to the ninth element of the name segment at `src/Billing/EDI270.php:L168`, under the payer branch that fixes the qualifier to payer identification at `src/Billing/EDI270.php:L166`. Both columns are selected by the same query that feeds the builder, at `src/Billing/EDI270.php:L410` and `src/Billing/EDI270.php:L411`, and are declared at `sql/database.sql:L3284` for the claims identifier and `sql/database.sql:L3290` for the eligibility identifier. The screen gates are three: the batch screen emits its request button only under the switch, at `interface/billing/edi_270.php:L440-L442`; the patient insurance card passes the switch to its template at `src/Patient/Cards/InsuranceViewCard.php:L62` and guards the real-time request path on it at `src/Patient/Cards/InsuranceViewCard.php:L164`, reaching the request itself at `src/Patient/Cards/InsuranceViewCard.php:L167`; and the payer administration screen shows the two eligibility columns only under the switch, assigned at `controllers/C_InsuranceCompany.class.php:L21` and consumed at `templates/insurance_companies/general_edit.html:L100` and `templates/insurance_companies/general_edit.html:L132`. VERIFIED: those three gates are not equivalent, and the difference between the first two is the part of this rule most easily got wrong. On the patient insurance card the switch is tested on the server before the request is made, at `src/Patient/Cards/InsuranceViewCard.php:L164`, with the request itself two lines inside that branch at `src/Patient/Cards/InsuranceViewCard.php:L167`, so with the switch off that path cannot transmit. On the batch screen the switch decides only whether the control is rendered, at `interface/billing/edi_270.php:L440-L442`: the branch that transmits tests the submitted button name and the presence of a result set and nothing else, at `interface/billing/edi_270.php:L164-L167`, and the only request check in the file is a cross-site request token, and only when the posted body is non-empty, at `interface/billing/edi_270.php:L40-L42`. So on that screen the switch gates the offer and not the server. The authorization gap that leaves is flagged, flag-only, as item A9 of [Appendix A of defect-candidates.md](defect-candidates.md#appendix-a-security-sensitive-observations).
- **Status:** VERIFIED
- **Intent:** INFERRED (confidence: High): the switch exists because eligibility requires a trading-partner relationship that most practices do not have, so the feature is hidden until a practice has one, and the identifier substitution exists because a payer that answers eligibility inquiries frequently identifies itself differently from the identifier used on claims. Basis: the switch's own label at `library/globals.inc.php:L2064` describes it as allowing eligibility checks using an X12 partner, which states the first half; the second half rests on the substitution being between two columns that the schema keeps separate at `sql/database.sql:L3284` and `sql/database.sql:L3290` and on the eligibility identifier being validated as mandatory on the eligibility path at `src/Billing/EDI270.php:L458-L460`, and no comment states the reason.
- **Novel:** yes - absent from Readme_edihistory.html, DEVELOPER_GUIDE.md, sl_eob_help.php and cms_1500_help.php.
- **Blast radius:** Turning the switch on without populating the eligibility identifier on each payer changes the identifier on the wire from a populated column to an empty one, so requests that previously reached a payer stop resolving. VERIFIED: the eligibility path does test for the empty case, accumulating a missing-payer-identifier error at `src/Billing/EDI270.php:L458-L460`, so this failure is reported rather than silent. VERIFIED: the reverse direction has no such test. Turning the switch off silently substitutes the claims identifier, which is populated for every payer the practice bills, so a well-formed 270 is built and transmitted carrying an identifier the eligibility endpoint was never told to expect; nothing on the path compares the two columns or warns that they differ. The two columns and why holding both is hazardous are set out in the payer identity reference in [transactions.md](transactions.md), and the segment the substituted value lands in is documented with the rest of the 270 in the same document. VERIFIED: the offer-versus-server asymmetry has a blast radius of its own, and the two halves of this rule compound in it. Because only one of the two entry points tests the switch on the server, turning eligibility off removes the feature from view without removing the ability to exercise it from the batch path; and because the identifier substitution is read when the segment is built rather than when the feature is configured, a request that does go out while the switch is off carries the claims identifier by the substitution above. So the configuration a practice would reach for to stop sending eligibility requests is also the configuration under which any request that still goes out is least likely to be recognised by the payer.
...
The register contains **72 rules**. Of those, **71 are VERIFIED** and **1 is INFERRED**.

| Status | Count | Detail |
|--------|------:|--------|
| VERIFIED | 71 | Traced in code, schema DDL or a test, each with a citation |
...
| I | Site-global-gated rules | 6 |
| **Total** | | **72** |

Two things about that distribution are worth stating rather than leaving to be noticed.

The first is that one entry-level INFERRED status does not mean the register contains one inference. Forty-seven of the seventy-one verified entries carry a labelled inference inside them, almost always in the `Intent:` field and occasionally about a consequence, and each of those carries its own confidence and its own one-line basis. The remaining twenty-four attribute their `Intent:` field to a comment, docblock, column comment, error message or help text that is cited in the field itself, so that no statement of purpose in this register is left as a bare assertion. The distinction the notation draws, and which [README.md](README.md) defines, is between a rule whose *existence and effect* are observed and a rule whose existence rests on reading rather than on tracing. Only one rule in this register is of the second kind, and it is the one where the sole direct statement of purpose is a docblock.

The second is that the confidence in a rule is not the confidence in its safety. Every entry in [Group F](#group-f-behaviour-that-would-silently-change-a-patient-or-payer-facing-amount-if-altered) is verified, which means the register is certain about what those eleven decisions do; it says nothing about whether they are correct. Fifteen entries cross-reference [defect-candidates.md](defect-candidates.md) precisely because a rule can be both faithfully recovered and wrong, and the two registers are meant to be read together at those points. That figure counts entry bodies carrying the cross-reference and not occurrences of the link in this document, three of which sit in the surrounding prose rather than inside an entry, and the fifteen are enumerated here so that the count can be checked rather than taken on trust: [BR-A3](#br-a3-two-coverage-money-fields-escape-the-fixed-point-convention), [BR-A8](#br-a8-a-non-numeric-adjustment-amount-becomes-zero), [BR-B2](#br-b2-provider-level-adjustments-are-excluded-from-ar-and-included-in-the-balance-test), [BR-B7](#br-b7-a-deposit-is-balanced-against-live-ledger-lines-only-and-only-in-a-browser-alert), [BR-C1](#br-c1-three-hardcoded-tables-carry-1391-x12-code-descriptions), [BR-C6](#br-c6-a-denial-is-status-7-and-is-recorded-unlike-any-other-status), [BR-D5](#br-d5-the-claim-identifier-is-recovered-from-the-remittance-by-counting-its-parts), [BR-E2](#br-e2-the-service-date-is-the-first-ten-characters-of-the-encounter-timestamp), [BR-F2](#br-f2-a-code-whose-colon-is-its-first-character-keeps-its-modifier), [BR-F3](#br-f3-a-failed-transmission-is-recorded-as-a-success), [BR-F6](#br-f6-the-attachment-segment-is-emitted-without-being-counted), [BR-F11](#br-f11-a-medicare-inpatient-adjudication-segment-ends-the-parse-where-it-appears), [BR-H1](#br-h1-the-claim-version-is-allocated-by-an-unlocked-aggregate-inside-a-transaction), [BR-H6](#br-h6-the-batch-renumbers-the-transaction-set-and-rewrites-the-reference-it-planted) and [BR-H8](#br-h8-envelope-validity-is-encoded-in-the-length-of-a-string).
...
|---------|------:|---------|
| Absent from all four existing sources | 66 | the remainder |
...
Rules were extracted from the conditional and arithmetic expressions themselves, because in this subsystem a rule is frequently expressed only as a comparison operator, a rounding choice or a whitelist membership test. Schema column comments were treated as first-class rule sources, because the database declares no check constraints anywhere across its 283 table definitions and several invariants are recorded only in comments. That denominator is a case-insensitive enumeration of `CREATE TABLE` that tolerates both leading whitespace and a doubled space between the keywords, which is what makes it catch the one indented declaration at `sql/database.sql:L1180` and the one written with two spaces at `sql/database.sql:L1778`; the same search for a `CHECK` clause returns nothing at all, so the absence is a property of every table and not of the 281 that a stricter pattern happens to find. Comments and docblocks in code were read as evidence of intent and never as evidence of behaviour, in accordance with the source-of-truth ordering defined in [README.md](README.md). Every behavioural claim carries a citation to the file and line range that supports it, and every citation was re-read against its target before publication. Nothing was executed: PHP and Composer are not installed in the authoring environment, so no claim here reports an observed run, and where a rule's consequence would need a run to confirm it, the consequence is labelled as an inference with its basis.
