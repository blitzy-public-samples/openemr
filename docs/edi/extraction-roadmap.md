...
- [E2 to E14 the remaining responsibilities](#e2-to-e14-the-remaining-responsibilities)
...
| Per-transaction rendering | `edih_835_html.php` 1,589, `edih_278_html.php` 916, `edih_271_html.php` 628, `edih_277_html.php` 307 | [E5](#e5-the-835-renderer), [E6](#e6-the-278-renderer), [E7](#e7-the-271-renderer); the 277 remnant is covered by E1 and E4 |
...
| [E4](#e4-retire-the-formatting-backfill) | Retire the formatting backfill | 107 call sites | nothing | Criterion 2 |
...
**The alternative, and why this document does not propose it.** The legacy symbol could be dropped and every site in the inventory below rewritten to the namespaced name in one commit. That is fewer moving parts and no shim, and it would be defensible. It is not proposed here for a stated reason rather than a stylistic one: four of the binding sites sit in procedural files that carry no namespace declaration at all, so each needs a fully qualified name or a first-ever `use` statement; the dependency-symbol whitelist entry and the static-analysis alias mirror both have to come out in the same commit; and the test file has to change in that commit too instead of in one of its own. Retaining the alias makes the change land in a single commit whose only behavioural surface is autoloading, and leaves rewriting the call sites as separate, individually revertible commits. An implementer who prefers the direct rewrite should note that it removes nothing from the call-site inventory below; it changes what is done at each site.

### E1 every call site to update

VERIFIED by repository-wide search across all PHP, XML, JSON and Markdown files outside `vendor/` and `node_modules/`. Eleven sites in nine files reference the symbol or its path, plus one test file and one configuration file. This inventory is deliberately complete rather than representative: an item that is meant to be executable without further scoping cannot leave a call site to be discovered during the work.

| # | Site | What is there | What the move does to it |
|--:|------|---------------|--------------------------|
| 1 | `src/Billing/EDI270.php:L27` | `use edih_271_codes;` | Replaced by `use OpenEMR\Billing\EdiHistory\Eligibility271Codes;`, or left as-is while the alias stands |
| 2 | `src/Billing/EDI270.php:L35` | `require_once(__DIR__ . "/../../library/edihistory/codes/edih_271_code_class.php");` | Deleted; the class is autoloaded once it is under `src/` |
| 3 | `src/Billing/EDI270.php:L930` | `$codes = new edih_271_codes('*', '^');` inside `parseEdi271()` at `src/Billing/EDI270.php:L927` | Constructor call renamed |
...
| 6 | `library/edihistory/edih_277_html.php:L96` | `$cd27x = new edih_271_codes($ds, $dr);` | Constructor call renamed, or unchanged under the alias |
| 7 | `library/edihistory/edih_271_html.php:L57` | `$cd271 = new edih_271_codes($ds, $dr);` | As above |
| 8 | `library/edihistory/edih_278_html.php:L53` | `$cd27x = new edih_271_codes($ds, $dr);` | As above |
| 9 | `library/edihistory/edih_835_html.php:L1506` | `$cd27x = new edih_271_codes($ds, $dr);` | As above |
| 10 | `library/edihistory/edih_835_html.php` three signatures | The type appears in three function signatures at `library/edihistory/edih_835_html.php:L25`, `library/edihistory/edih_835_html.php:L262` and `library/edihistory/edih_835_html.php:L810`, each as `edih_271_codes $codes27x, edih_835_codes $codes835` | Type name changed in three places, or unchanged under the alias |
| 11 | `interface/billing/edih_main.php:L84` | `require_once($srcDir . '/edihistory/codes/edih_271_code_class.php');` | Retained if the shim stays, since the shim is not autoloaded either; deleted if the alias is dropped |
...
- Change nothing else. The 421 lines of assertions are the regression net for this move and their expectations must not be adjusted to fit it: if an assertion fails after the move, the move is wrong.

**Proposed new test.** A dedicated test for the table itself, proposed at `tests/Tests/Isolated/Billing/EdiHistory/Eligibility271CodesTest.php`, sized to the clerical risk rather than to the line count. Four cases cover it:
...
Seven criteria, each checkable by a named command or a named observation. The item is done when all seven hold and not before.
...
## E2 to E14 the remaining responsibilities
...
- **Acceptance criteria.** A repository-wide search for `edih_format_date`, `edih_format_money` and `edih_format_percent` returns neither a definition nor a call, because all 107 sites across the four legacy files that hold them now name `EdiFormat` and the three delegate definitions are gone from `library/edihistory/edih_csv_inc.php`; rendering a 271, 277, 278, 835 and acknowledgement produces byte-identical HTML to a capture taken before the change; the seven baseline entries named under blocking dependencies are deleted and static analysis passes at `level: 10` per `phpstan.neon.dist:L9` with no unmatched ignored errors; `EdiFormatTest` passes under `phpunit-isolated.xml` with the telephone cases added.
...
- **Blocking dependencies.** E1, E2 and E4, in that order and all three. E1 and E2 because both code tables appear in every one of its three signatures; E4 because 79 of the 107 formatting call sites are in this file, including all 6 of the telephone sites, and rewriting them during a segment extraction would hide the extraction inside a mechanical diff.
...
- **Blocking dependencies.** E1 and E4. E1 because it constructs the 271 code table at `library/edihistory/edih_278_html.php:L53`; E4 because 16 of the 107 formatting sites are in this file.
- **Proposed test surface.** A dedicated test proposed at `tests/Tests/Isolated/Billing/EdiHistory/ServicesReview278RendererTest.php`, one data-provider case per extracted method. One case is proposed by name: the dental service segments the file's own header says it ignores, at `library/edihistory/edih_278_html.php:L31`, asserted as producing no output, so that a later decision to support them is a visible change rather than a silent one.
- **Acceptance criteria.** Rendering the 278 corpus fixtures produces byte-identical HTML; the new test covers every extracted method; the ignored-segment behaviour is pinned; the legacy entry-point signatures are unchanged; static analysis passes with no new baseline entries for the new file.
...
- **Blocking dependencies.** E1 and E4. E1 because it constructs the code table at `library/edihistory/edih_271_html.php:L57`; E4 because 11 of the 107 formatting sites are in this file.
- **Proposed test surface.** A dedicated test proposed at `tests/Tests/Isolated/Billing/EdiHistory/Eligibility271RendererTest.php`, one data-provider case per extracted method, with a case for a benefit segment carrying a percentage value, because that is the one path in this renderer that reaches `EdiFormat::percent()` and therefore the one whose output depends on E4 having been done correctly. VERIFIED: the file has exactly one such call site, the EB08 benefit percentage at `library/edihistory/edih_271_html.php:L367`.
- **Acceptance criteria.** Rendering the 271 corpus fixtures produces byte-identical HTML; the new test covers every extracted method; the legacy entry-point signatures are unchanged; static analysis passes with no new baseline entries for the new file.
...
- **Acceptance criteria.** A test for `Recorder` exists and passes before any behaviour moves; after the move, posting a remittance through the EOB screen produces the same rows in `ar_session` and `ar_activity` as before, compared field by field; both deprecated helpers still accept their existing positional arguments so no caller changes; the chosen resolution of the two session-writer shapes is recorded in the class docblock with the behaviour that changed; no monetary value differs by a cent in the comparison.
...
The table below is the proposed inventory. Every fixture is a proposal; the citation in the final column is VERIFIED and is the reason the case belongs in the corpus.
...
| `271-response-unrequested-set` | 271 eligibility response | At least one EB code from an element set no caller requests | Same evidence. The 46 unrequested sets are the part of E1 that no test would notice breaking, so the corpus has to reach one deliberately |
| `276-inquiry` | 276 claim-status request | TRN trace-number segments at the levels the request carries | VERIFIED in [transactions.md](transactions.md): the 276 has no renderer of its own and shares the 277 path, so E7 and E1 both touch it while nothing exercises it today |
...
| `envelope-isa-not-105` | Any | An interchange header that is not 105 characters | The negative case for the same check. VERIFIED: the failure is `die()` at `src/Billing/BillingProcessor/BillingClaimBatch.php:L221`, and a second `die()` at `:L225-L226` covers content that does not begin with `ISA` at all. A stored fixture is how a follow-on run demonstrates that a replacement returns instead of terminating the request |
...
Two properties of that inventory are deliberate and worth stating so a follow-on run does not trim them. First, the four cases the corpus exists for - `835-mia`, `835-plb`, `envelope-isa-not-105` and `envelope-multi-gs` - are cases the current code gets wrong or handles abruptly, and each is registered as a defect candidate in [defect-candidates.md](defect-candidates.md). Storing them now means a later fix can be proved rather than asserted: the golden file changes, the diff is reviewed, and the change is the evidence. Second, `271-response-unrequested-set` exists to cover the part of E1 that has no consumer, which is the part a coverage report would rank last and a refactor would break first.
...
**Inbound comparison needs no normalisation at all.** VERIFIED: neither the remittance parser nor any renderer reads the current clock. A search of `src/Billing/ParseERA.php`, `src/Billing/EdiHistory/Claim277Renderer.php` and all five legacy renderers for a current-time read - a bare `time()`, a single-argument `date()`, `strtotime('now')` or `new DateTime()` - returns no match in any of the seven files. Every date-shaped call formats a value taken out of the input, for example `EdiFormat::date($sar[4])` at `src/Billing/EdiHistory/Claim277Renderer.php:L87` and `edih_format_date($sar[2])` at `library/edihistory/edih_835_html.php:L355`. So for E1 through E9, which are all inbound, the comparison is the simple one: the same fixture in, the same bytes out, with no exception list to maintain.
...
`phpunit-isolated.xml`, for the same reason [E1 proposed test surface](#e1-proposed-test-surface) gives. VERIFIED: the isolated suite is declared at `phpunit-isolated.xml:L64-L67`; a dedicated workflow invokes it at `.github/workflows/isolated-tests.yml:L50` across a PHP 8.2 to 8.6 matrix declared at `.github/workflows/isolated-tests.yml:L30-L35`; and `composer.json:L310` names the shortcut that runs it. VERIFIED: that configuration fails on deprecations and on PHPUnit warnings, at `phpunit-isolated.xml:L26-L28`, so a corpus test that provokes one fails rather than passing with a notice - which is a property to design for, because the legacy tree being loaded under test is what tends to emit them.

Naming the configuration is not a formality. VERIFIED: the primary suite list at `phpunit.xml:L43-L93` does not include the isolated suite, so a reader who ran only the primary configuration would conclude the corpus does not exist.

One split in the inventory follows from this and should be decided before any fixture is written rather than discovered halfway through:

- **The inbound fixtures are isolated-safe as they stand.** Every one of them is a file in and text or a parsed structure out, and both existing inline X12 tests already run under the isolated configuration.
...
flowchart LR
    ...
    EDI270 -- "constructs through the alias" --> SHIM
```
