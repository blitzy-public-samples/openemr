...
Inbound only. VERIFIED: nothing in this codebase generates an acknowledgement, and nothing in it receives an interchange from a trading partner that would call for one - the outbound direction is claim submission and the inbound direction is parsed and displayed, never acknowledged.

INFERRED (confidence: High): the absence of a generator is the correct design here rather than a gap, because an acknowledgement is produced by the receiver of an interchange and this system acts only as a submitter. Basis: the X12 acknowledgement transactions report on a received interchange, and every transmission path in this subsystem sends rather than receives interchanges, so there is no interchange for this system to acknowledge. This is a judgement about what the system ought to do, not an observation of what it does, which is why it is separated from the verified statement above.
...
VERIFIED: none of the five is read by anything other than the model that declares them, which is the fact that makes them dead regardless of what they were for.
...
| The two-generation split of the 277 renderer is the residue of an extraction, the case bodies having been lifted out and the control flow left behind | High | The halves partition one renderer without overlap, eleven public methods correspond to the legacy switch cases, and seven of them are type-hinted against a legacy class | `src/Billing/EdiHistory/Claim277Renderer.php:L58-L336`, `library/edihistory/edih_277_html.php:L40` |
| The absence of an acknowledgement generator is the correct design rather than a gap | High | An acknowledgement reports on a received interchange and every transmission path here sends rather than receives one; this is a judgement about what the system ought to do | `library/edihistory/edih_997_error.php:L69-L80` |
...
Fourteen inferences, of which eight are High confidence, five are Medium and one is Low. Each corresponds to exactly one `INFERRED` label in the body above. Every other statement this document makes about the code was verified against the cited range at the recorded commit.
