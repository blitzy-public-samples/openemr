...
**Where that endpoint value comes from is a trust boundary, and it is worth stating beside the column that supplies it.** VERIFIED: the partner row is read whole by `EDI270::getX12Partner()`, declared at `src/Billing/EDI270.php:L744` and selecting every column at `src/Billing/EDI270.php:L752`; the endpoint column is taken as the request URL at `src/Billing/EDI270.php:L798` and posted to at `src/Billing/EDI270.php:L852` with no scheme, host or destination check. VERIFIED: the column reaches that state through the same bare setter shape as the rest of the table - the controller base assigns any posted field with a matching setter, at `library/classes/Controller.class.php:L98-L115`, the setter stores the value as given, at `library/classes/X12Partner.class.php:L467-L470`, and the controller persists it, at `controllers/C_X12Partner.class.php:L67-L85`. Whoever can edit a trading-partner row therefore chooses the network destination this request is sent to. This is flagged only. The security-facing treatment belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction it is recorded and not analysed further.
...
**What guards the screen that sends this request is worth recording beside it.** An **ACL**, access control list, is the per-feature permission check an OpenEMR screen performs by calling `AclMain::aclCheckCore()`. VERIFIED: the eligibility screen requires an active session at `interface/billing/edi_270.php:L38` and checks a cross-site request token on the method that carries its work at `interface/billing/edi_270.php:L41`, and performs no endpoint-local feature ACL of its own; a repository search of that file for `aclCheckCore` returns nothing. It is not unauthenticated. VERIFIED: the permission requirement associated with it lives in menu metadata, at `interface/main/tabs/menu/menus/standard.json:L1545-L1567`, which governs whether the menu entry is shown; a menu entry's visibility requirement is not the endpoint's own check, because it does not run when the endpoint's URL is requested directly. The same distinction across all six revenue-cycle entry points is tabulated in [claim-lifecycle.md](claim-lifecycle.md#access-control-at-the-stage-entry-points). This is flagged only. The security-facing treatment belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction it is recorded and not analysed further.
...
**Whether a stored 271 archive is ciphertext is a property of the site's configuration rather than of this code path, and this document says "accessor" rather than "encrypted" for that reason.** VERIFIED: the accessor returns the file's bytes unchanged when the site's filesystem-encryption setting is off, at `src/BC/Crypto/ContextualEncryptionTrait.php:L36-L37`, so what `interface/billing/edi_271.php:L60` writes is then the cleartext 271; the read side is symmetrically indifferent, returning its argument unchanged when the stored value carries no encryption marker, at `src/BC/Crypto/ContextualEncryptionTrait.php:L62-L64`, so both states parse identically and nothing on the path reports which state a given stored archive is in. VERIFIED: this screen too requires an active session at `interface/billing/edi_271.php:L32` and checks a request token at `interface/billing/edi_271.php:L34` while performing no endpoint-local feature ACL of its own, on the same menu-metadata-only footing as the request screen above. Both observations are flagged only. The security-facing treatment belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction they are recorded and not analysed further.
...
**One output-encoding gap sits in the index listing that reaches this renderer, and naming the exact fields matters because most of that listing is encoded.** VERIFIED: the branch that lists files of this type, opened at `library/edihistory/edih_csv_data.php:L873`, encodes three named columns individually - the file date, the transaction-set reference and the file name - and then falls through to an unencoded cell for every remaining column, at `library/edihistory/edih_csv_data.php:L891-L893`. VERIFIED: the columns reaching that fall-through include the payer name and the patient name, which the index builder composed from the NM1 name segments of the file itself, at `library/edihistory/edih_csv_parse.php:L862-L876`, NM1 being the X12 name segment. Those are the specific unencoded fields; the rest of the listing wraps its values, so this is not a statement that legacy display output is raw in general. This is flagged only. The security-facing treatment belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction it is recorded and not analysed further.
...
**One output-encoding gap sits in this renderer, and naming the exact fields matters because the descriptions printed beside them are encoded.** VERIFIED: the summary builder emits the AK9 element values themselves without passing them through the output-encoding helper, at `library/edihistory/edih_997_error.php:L234-L242` - the acknowledgement code and the five group-level error codes are concatenated into the markup directly and the three transaction-set counts are string-interpolated into it, while only the looked-up English descriptions beside them are encoded. AK9 is the functional group response trailer, whose elements carry the accept-or-reject flag, the submitted, received and accepted transaction-set counts, and up to five group-level error codes. Those are the specific unencoded fields; the TA1, AK3 and AK5 values rendered elsewhere in the same file are not part of this observation, so this is not a statement that the report as a whole is raw. VERIFIED: the printable-character scan an upload passes on the way in is not output encoding and does not substitute for it - it admits or refuses a whole file on a character-class and markup-fragment test, at `src/Billing/EdiHistory/X12File.php:L318-L322`, and this report is built from the parsed object afterwards. This is flagged only. The security-facing treatment belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction it is recorded and not analysed further.
...
| `x12_sftp_pass` | `sql/database.sql:L10047` | null | The file-transfer password, and the CAQH password part. Encrypted by the application on write, but only while the `database_encryption` global is on; it defaults to on and is operator-configurable | Live | `src/Billing/BillingProcessor/X12RemoteTracker.php:L90`, `src/Billing/EDI270.php:L793` |
...
Two notes on that table before the columns that need their own subsections.

VERIFIED: two of these columns hold credentials, and the difference between them is what the code attempts rather than what the schema guarantees. `x12_isa04`, described by its own schema comment as the user password at `sql/database.sql:L10035`, is stored in clear and returned as stored for emission into ISA04 of every claim interchange, at `src/Billing/Claim.php:L693-L695`. The file-transfer password at `sql/database.sql:L10047` is the one credential column with an encryption attempt on its write path, at `controllers/C_X12Partner.class.php:L79-L83`, and that attempt is conditional on the site's `database_encryption` global, declared as a boolean defaulting to on at `library/globals.inc.php:L1028-L1032`, rather than being an invariant of the column. Both are plain character types and the schema declares nothing about either. This is flagged only, and deliberately not traced further here: the security-facing treatment of both columns belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction it is recorded and not analysed further.
...
**The four transport columns are a trust boundary, and this is the point in the reference where that is worth saying.** VERIFIED: all four are stored through the same bare setter shape as the rest of the table - the controller base assigns any posted field with a matching setter, at `library/classes/Controller.class.php:L98-L115`, the setters store the value as given, at `library/classes/X12Partner.class.php:L162-L199`, and the controller persists it, at `controllers/C_X12Partner.class.php:L67-L85`. VERIFIED: the stored values then become a local filesystem path by direct concatenation at `src/Billing/BillingProcessor/X12RemoteTracker.php:L75`, a directory created recursively at `src/Billing/BillingProcessor/Tasks/GeneratorX12Direct.php:L102`, a network destination at `src/Billing/BillingProcessor/X12RemoteTracker.php:L89`, and a remote directory change at `src/Billing/BillingProcessor/X12RemoteTracker.php:L99`, with no format, destination or path check at any of the four. Whoever can edit a trading-partner row therefore chooses a server filesystem path and a network destination for the outbound path, the same boundary the eligibility endpoint column crosses for the real-time path above. This is flagged only. The security-facing treatment belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction it is recorded and not analysed further.
