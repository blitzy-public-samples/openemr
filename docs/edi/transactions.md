...
One property of the screen that consumes those columns is worth naming, because it decides which configuration reaches the wire. VERIFIED: the site switch `enable_eligibility_requests` decides only whether the transmit control is rendered on the batch eligibility screen, at `interface/billing/edi_270.php:L440-L442`, and is not tested in the transmit branch at `interface/billing/edi_270.php:L164-L167`, so on this screen it gates the offer and not the handler - the asymmetry registered as [BR-I6](business-rules.md#br-i6-one-switch-decides-what-the-interface-offers-and-which-payer-identifier-the-request-carries). VERIFIED: that branch is where the three real-time columns are consumed - the endpoint at `src/Billing/EDI270.php:L798`, the transport login at `src/Billing/EDI270.php:L794` and the transport password at `src/Billing/EDI270.php:L793` - which is what makes them live rather than dead. The access-control reading of the same branch is flagged, flag-only, as item A9 of [Appendix A of defect-candidates.md](defect-candidates.md#appendix-a-security-sensitive-observations); it is a pre-existing condition of the source and is not pursued here.
...
| `x12_isa04` | `sql/database.sql:L10035` | Ten spaces | ISA04. Comment describes it as the user password. Emitted on every claim interchange. Flagged as item A3 of [Appendix A](defect-candidates.md#appendix-a-security-sensitive-observations) | Live on claims only | `src/Billing/Claim.php:L695` |
...
| `x12_sftp_pass` | `sql/database.sql:L10047` | null | The file-transfer password, and the CAQH password part. Written by the partner-edit controller at `controllers/C_X12Partner.class.php:L79-L83`. Flagged as item A4 of [Appendix A](defect-candidates.md#appendix-a-security-sensitive-observations) | Live | `src/Billing/BillingProcessor/X12RemoteTracker.php:L90`, `src/Billing/EDI270.php:L793` |
...
| `x12_client_secret` | `sql/database.sql:L10057` | null | Nothing. Flagged as item A5 of [Appendix A](defect-candidates.md#appendix-a-security-sensitive-observations) | Dead | `library/classes/X12Partner.class.php:L447-L454` |
...
**That administrative route is named here, and only named, because this is the section in which a reader learns it exists.** It is the route on which every column in [The column reference](#the-column-reference) is edited and stored, credential columns included, which is why the generic setter walk cited above makes the endpoint columns writable as well as readable even though nothing consumes their values. Three security-sensitive observations on the same route are flagged, flag-only and with no analysis beyond the one line each, as items A6, A7 and A8 of [Appendix A of defect-candidates.md](defect-candidates.md#appendix-a-security-sensitive-observations). All three are pre-existing conditions of the source; nothing in this documentation set changes any of them, and this document does not pursue them further.
