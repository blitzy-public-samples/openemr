...
VERIFIED: the file-transfer password at `sql/database.sql:L10047` is the one credential column the code attempts to encrypt, and the attempt is conditional rather than an invariant. Its single write site encrypts it at `controllers/C_X12Partner.class.php:L79-L83`, but the method called there returns its argument unchanged when a site flag is off, at `src/BC/Crypto/ContextualEncryptionTrait.php:L25-L27`, and that flag is taken from the `database_encryption` global at `src/Common/Crypto/CryptoGen.php:L81-L82`. VERIFIED: that global is declared as a boolean defaulting to on and is operator-configurable, at `library/globals.inc.php:L1028-L1032`. A site that turns it off stores this password in clear in the column.

VERIFIED: nothing on the path detects or reports that state. Both readers call the decrypting accessor - the transport at `src/Billing/BillingProcessor/X12RemoteTracker.php:L90` and the eligibility request at `src/Billing/EDI270.php:L793` - and that accessor returns its argument unchanged when the stored value carries no encryption marker, at `src/BC/Crypto/ContextualEncryptionTrait.php:L47-L48`. A plaintext password therefore authenticates exactly as an encrypted one does, with no error, no warning and no log entry distinguishing them. The edit screen decrypts symmetrically for display, at `controllers/C_X12Partner.class.php:L49-L52`.

The difference between this column and the interchange password column at `sql/database.sql:L10035` is therefore a difference in what the code attempts, not a storage guarantee: this one is encrypted under a default-on setting that can be switched off, while that one is never encrypted at all. Both are plain character types, and the schema declares nothing about either. The plaintext interchange column is flagged in [defect-candidates.md](defect-candidates.md).
...
Fifteen inferences, of which seven are High confidence, five are Medium and three are Low. Each corresponds to exactly one `INFERRED` label in the body above. Every other statement this document makes about the code was verified against the cited range at the recorded commit.
