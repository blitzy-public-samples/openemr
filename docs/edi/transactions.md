...
For each X12 transaction type this codebase handles, where it is built or read, what is non-obvious about the way it is built or read, and which trading-partner configuration changes it.
...
| `HR` | 276 | Outbound | Generic segment display at `library/edihistory/edih_io.php:L445` |
...
Every one of those literals except ISA11, ISA12, ISA16, GS01, GS07, ST01 and BHT01 is overwritten downstream, because a single claim is never transmitted on its own: the batch post-processor rewrites the interchange, group and transaction-set envelopes and reissues the trailers. That rewriting, element by element and with its own citations, is stage S5 of [claim-lifecycle.md](claim-lifecycle.md) and is not duplicated here. The relevant fact for this section is that the generator's own values are placeholders by design for four of them and by accident for none.
...
The reason is architectural rather than accidental. Outbound 837 generation belongs entirely to the modern namespace and never had a legacy counterpart, so there was never a legacy renderer to write: the legacy tree indexes the batch file the modern pipeline produces, reading it out of the outbound directory recorded at `library/edihistory/edih_csv_inc.php:L738`, and displays it as raw segments. A reader looking for the legacy 837 renderer should stop looking.
...
- Four envelope control numbers are fixed literals in the generator and depend on a downstream rewriting pass to become correct: ISA13 at `src/Billing/X125010837P.php:L73`, GS06 at `src/Billing/X125010837P.php:L85`, ST02 at `src/Billing/X125010837P.php:L103` and BHT03 at `src/Billing/X125010837P.php:L111`. A generated claim inspected before that pass is not a valid interchange.
- The GS06 literal carries an unresolved note asking for a tracking number, at `src/Billing/X125010837P.php:L85`.
...
The institutional envelope differs from the professional one in four positions, and every difference makes the institutional envelope more nearly correct before post-processing.
...
The same envelope columns as the 837P, read through the same claim accessors, with two exceptions. VERIFIED: the institutional generator does not consult `x12_submitter_name` at all, so the loop 1000A submitter substitution described in the professional section does not apply to it; and it does not participate in the per-partner direct path, so `x12_sftp_local_dir` does not affect where its output is written. Every other row of the professional table above applies unchanged.
...
Because that return abandons the parse rather than skipping the segment, the operational consequence is not partial data: **a Medicare Part A remittance containing an MIA segment can be viewed in full in the EDI history browser and can never be posted to accounts receivable.** Generation one of this subsystem is, in this one respect, more capable than generation two, which inverts the assumption that newer code supersedes older. The rule this embodies belongs to [business-rules.md](business-rules.md); the suspicion that it is a defect rather than a decision belongs to [defect-candidates.md](defect-candidates.md); the stage at which the failure becomes visible, and what the operator sees, is stage S10 of [claim-lifecycle.md](claim-lifecycle.md).
...
|---------|----------------|---------------|--------|
| MIA, inpatient adjudication | Recognised and rendered, at `library/edihistory/edih_835_html.php:L531` | No branch; falls to the catch-all at `src/Billing/ParseERA.php:L467-L468` | The entire file is rejected and nothing posts |
| RDM, remittance delivery method | Recognised and rendered, at `library/edihistory/edih_835_html.php:L1111`, decoding the by-mail code at `library/edihistory/edih_835_html.php:L1117-L1118` | No branch; falls to the same catch-all | The entire file is rejected and nothing posts |
...
**Provider-level adjustments are read and deliberately excluded from accounts receivable.** VERIFIED: the PLB branch at `src/Billing/ParseERA.php:L429-L440` loops the adjustments at `src/Billing/ParseERA.php:L432` and turns each into a warning string at `src/Billing/ParseERA.php:L437-L438` rather than into a postable amount, and the comment above it at `src/Billing/ParseERA.php:L430-L431` states that these belong to the general ledger rather than to accounts receivable. That exclusion conflicts with how the modern balance test treats the same money, and the conflict is registered as a rule in [business-rules.md](business-rules.md) and as a defect candidate in [defect-candidates.md](defect-candidates.md); it is named here only so that a reader of this section knows the exclusion is intentional at this end.
...
This is the only inbound transaction with a purpose-built renderer of its own that is not shared with another transaction type.

### Test coverage

One test file, covering the full parse only. VERIFIED: `tests/Tests/Isolated/Billing/ParseERATest.php` is 256 lines, calls only `ParseERA::parseERA()` at `tests/Tests/Isolated/Billing/ParseERATest.php:L129`, and holds nine tests at `tests/Tests/Isolated/Billing/ParseERATest.php:L169-L248` concerned with the numeric typing of parsed amounts and with adjustment group codes. It runs under the isolated configuration at `phpunit-isolated.xml:L65-L67`. Its fixtures declare an implementation guide identifier in their GS segment, at `tests/Tests/Isolated/Billing/ParseERATest.php:L34-L55`, which the parser does not read.
...
**The service date in DTP03 is the one genuinely partner-configurable value on this path.** VERIFIED: `createDTP()` at `src/Billing/EDI270.php:L250-L269` first special-cases the qualifier for a plan begin date at `src/Billing/EDI270.php:L256-L257`, and otherwise selects the date by matching the partner's date-type column at `src/Billing/EDI270.php:L259-L263`: the appointment date for `A` at `src/Billing/EDI270.php:L260`, the insurance effective date for `E` at `src/Billing/EDI270.php:L261`, and today for anything else at `src/Billing/EDI270.php:L262`, with the chosen value written into DTP03 at `src/Billing/EDI270.php:L264`. The trading-partner model offers three values for that column at `library/classes/X12Partner.class.php:L428-L435`, of which the third, current date, has no explicit branch and reaches the default. VERIFIED: the default is today, so the behaviour for the current-date option is correct by accident rather than by design.
...
VERIFIED: three of those part names use a lower-case `d` in their identifier suffix, at `src/Billing/EDI270.php:L809`, `src/Billing/EDI270.php:L825` and `src/Billing/EDI270.php:L829`. The names above reproduce the source exactly, because a CAQH CORE part name is matched case-sensitively by the receiving system.

VERIFIED: the response is checked for integrity at `src/Billing/EDI270.php:L856-L857` and then split by a multipart reader at `src/Billing/EDI270.php:L858`, with error text assembled at `src/Billing/EDI270.php:L860-L873`. Error detection in the calling screen is by substring: the dispatching wrapper looks for the literal text of an error prefix and, failing that, for the three letters of the request-validation segment identifier, at `src/Billing/EDI270.php:L429-L430`.
...
**None, and why.** VERIFIED: there is no 270 renderer, and the 270 does not share the 271 renderer either, which is worth stating explicitly because sharing would be the natural assumption. The 271 renderer's file-level entry point hardcodes the eligibility-response type when it loads a parsed file, at `library/edihistory/edih_271_html.php:L574`, so it cannot be handed a 270. The routing branch confirms it: the eligibility, status and authorisation family is routed at `library/edihistory/edih_io.php:L435-L446`, where only the status response, the eligibility response and the authorisation response have named renderers, and the remaining two types, the 270 and the 276, fall to the else branch at `library/edihistory/edih_io.php:L443`, whose own comment at `library/edihistory/edih_io.php:L444` says that HTML display is not available and segments are to be used instead. A 270 in the history browser is displayed as raw segments by `edih_display_text()` at `library/edihistory/edih_io.php:L445`.

### Test coverage

`tests/Tests/Isolated/Billing/EDI270Test.php`, 274 lines, running under the isolated configuration at `phpunit-isolated.xml:L65-L67`. VERIFIED: its 20 tests at `tests/Tests/Isolated/Billing/EDI270Test.php:L71-L268` exercise the transaction set header, the hierarchical transaction header, all three hierarchical level shapes, both reference flavours, the demographic segment, the date segment across all three partner date-type values and both qualifiers, the inquiry segment, all three trailers, and the relationship translator at `src/Billing/EDI270.php:L320-L324`.

VERIFIED by inspection of that file: the interchange header builder, the functional group header builder, the name builder, the unreachable trace-number builder, both assembly paths, the real-time request and the response parser are not exercised by it. This is the best-covered builder in the subsystem and its coverage still stops at the envelope.
...
INFERRED (confidence: High): the 276 support that exists was written to file inquiries produced by an external tool or by a clearinghouse portal, not to file inquiries this system sent. Basis: the legacy index classifies the type as submitted rather than as a response, at `library/edihistory/edih_csv_data.php:L65` and `library/edihistory/edih_csv_data.php:L612`, and keys it by a request date at `library/edihistory/edih_csv_inc.php:L746`, which is the treatment given to the 837 and the 270; yet unlike those two there is no code that produces the file being filed.
...
`none`. VERIFIED: no test anywhere under `tests/` refers to the 276. The only 276-adjacent coverage is the file reader's own type-mapping test, `tests/Tests/Isolated/Billing/EdiHistory/X12FileIsolatedTest.php:L173`, which asserts that the dispatch map resolves the known functional group codes, and which therefore covers the map entry without covering any 276 behaviour.

### Known gaps

- No generator. The transaction the whole indexing apparatus is built to file cannot be produced by this system.
- No renderer.
- The trading-partner claim-status endpoint column that would carry a 276 over HTTP is read by nothing.
...
This is the only rendering path in the subsystem that spans two generations, and its shape is evidence about how the extraction proceeded: the legacy file kept the loop, the switch and the state cursor, and the case bodies moved out. VERIFIED: the extracted class is larger than the legacy file that remains, 373 lines against 307.

### Test coverage

`tests/Tests/Isolated/Billing/EdiHistory/Claim277RendererTest.php`, 421 lines, running under the isolated configuration at `phpunit-isolated.xml:L65-L67`. VERIFIED: it covers all eleven public methods across twenty-four tests at `tests/Tests/Isolated/Billing/EdiHistory/Claim277RendererTest.php:L47-L417`, including a test that the constructor is private at `tests/Tests/Isolated/Billing/EdiHistory/Claim277RendererTest.php:L415`. It is the best-covered file in the subsystem.
...
- Nothing persists a 277: the transaction is displayed and indexed, and no claim status column anywhere is updated from it. The tables a claim's status does move through are in stage S8 of [claim-lifecycle.md](claim-lifecycle.md).
...
**Two segment families are documented as deliberately skipped.** VERIFIED: the docblock at `library/edihistory/edih_278_html.php:L30-L31` records that the dental service line and the tooth identification segments are ignored. Under the source-of-truth ordering that is a statement of intent; what the code does is simply to have no case for either identifier, which the enumeration above confirms.
...
Inbound only. Nothing in this codebase generates an acknowledgement, which is correct: an acknowledgement is produced by the receiver of an interchange, and this system is a submitter.
...
`none`. VERIFIED: no test anywhere under `tests/` refers to `edih_997_error` or to `edih_997_codes`. The only adjacent coverage is the file reader's acceptance of either acknowledgement number, exercised indirectly by the type-mapping tests at `tests/Tests/Isolated/Billing/EdiHistory/X12FileIsolatedTest.php:L173` and `tests/Tests/Isolated/Billing/EdiHistory/X12FileIsolatedTest.php:L184`.
...
| `id_number` | `sql/database.sql:L10028` | null | Loop 1000B receiver identification code, with dashes stripped | Live | `src/Billing/Claim.php:L969` |
...
| `processing_format` | `sql/database.sql:L10031` | null | No envelope element. Read per claim and stored with it | Live, purpose contested | `src/Billing/BillingProcessor/BillingClaim.php:L137-L142` |
...
| `x12_isa04` | `sql/database.sql:L10035` | Ten spaces | ISA04. Comment describes it as the user password. Stored in clear | Live on claims only | `src/Billing/Claim.php:L695` |
...
| `x12_submitter_id` | `sql/database.sql:L10044` | null | The key used to read the submitter contact block from `users` | Live on claims only | `src/Billing/Claim.php:L826`, `src/Billing/Claim.php:L838`, `src/Billing/Claim.php:L861`, `src/Billing/Claim.php:L875` |
| `x12_submitter_name` | `sql/database.sql:L10045` | null | The loop 1000A submitter name, gated by a site global | Live on claims only | `src/Billing/Claim.php:L660` |
...
| `x12_sftp_pass` | `sql/database.sql:L10047` | null | The file-transfer password, and the CAQH password part. Encrypted at rest | Live | `src/Billing/BillingProcessor/X12RemoteTracker.php:L90`, `src/Billing/EDI270.php:L793` |
| `x12_sftp_host` | `sql/database.sql:L10048` | null | The file-transfer host | Live | `src/Billing/BillingProcessor/X12RemoteTracker.php:L89` |
| `x12_sftp_port` | `sql/database.sql:L10049` | null | The file-transfer port | Live | `src/Billing/BillingProcessor/X12RemoteTracker.php:L89` |
| `x12_sftp_local_dir` | `sql/database.sql:L10050` | null | Where a per-partner batch file is written and later read from | Live | `src/Billing/BillingProcessor/Tasks/GeneratorX12Direct.php:L115`, `src/Billing/BillingProcessor/X12RemoteTracker.php:L75`, `interface/billing/get_claim_file.php:L71` |
| `x12_sftp_remote_dir` | `sql/database.sql:L10051` | null | The directory changed into on the partner's server | Live | `src/Billing/BillingProcessor/X12RemoteTracker.php:L99` |
...
| `x12_client_secret` | `sql/database.sql:L10057` | null | Nothing | Dead | `library/classes/X12Partner.class.php:L447-L454` |
...
Four notes on that table before the columns that need their own subsections.

VERIFIED: `x12_isa04` holds a credential in clear text. Its own schema comment at `sql/database.sql:L10035` describes it as the user password, it has no encryption on write and none on read at `src/Billing/Claim.php:L695`, and it is emitted verbatim into ISA04 of every claim interchange. This is flagged only. The security-facing treatment belongs to the appendix of [defect-candidates.md](defect-candidates.md), where by explicit instruction it is recorded and not analysed further.

VERIFIED: the file-transfer password at `sql/database.sql:L10047` is the one credential column that is encrypted at rest, because both of its readers decrypt it: the transport at `src/Billing/BillingProcessor/X12RemoteTracker.php:L90` and the eligibility request at `src/Billing/EDI270.php:L793`. The contrast with the interchange password column is a property of the code, not of the schema, since both columns are plain character types.

VERIFIED: nine columns are read by exactly one of the two outbound paths and not by the other. Eight are claims-only, being the four interchange authorisation and security columns `x12_isa01` through `x12_isa04`, the two application sender and receiver codes `x12_gs02` and `x12_gs03`, and the two submitter columns `x12_submitter_id` and `x12_submitter_name`. The ninth, the date-type column `x12_dtp03`, is eligibility-only. Each verdict was taken by searching `src/`, `library/` and `interface/` for the column name and excluding the trading-partner model, and in every one of the nine cases the search returned reads from one path only. A partner configured for claims is therefore not fully configured for eligibility, and the reverse.
...
**Mechanism one, five columns with an accessor and no consumer.** VERIFIED: the token endpoint at `sql/database.sql:L10052`, the claim status endpoint at `sql/database.sql:L10054`, the attachment endpoint at `sql/database.sql:L10055`, the client identifier at `sql/database.sql:L10056` and the client secret at `sql/database.sql:L10057` are each read only by the getter and setter pair the trading-partner model declares for them, at `library/classes/X12Partner.class.php:L437-L494`. Nothing anywhere else in `src/`, `library/`, `interface/` or `tests/` reads any of the five.

Together they form one coherent unimplemented feature: a client identifier and secret to obtain a token from a token endpoint, in order to call a claim-status endpoint and an attachment endpoint. That is the shape of an OAuth-secured HTTP integration for the 276 and for medical attachments, and neither exists.
...
Two consequences of the dead five are visible elsewhere in this document. The dead attachment endpoint is why the professional generator can promise a payer an electronically transmitted attachment at `src/Billing/X125010837P.php:L786-L792` and never send one. The dead claim-status endpoint is why the 276 has no generator at all, as [276 Claim Status Inquiry](#276-claim-status-inquiry) sets out.
...
| `enable_eligibility_requests` | `library/globals.inc.php:L2060-L2065` | `0` at `library/globals.inc.php:L2063` | Selects which `insurance_companies` payer identifier the 270 writes into NM109 | `src/Billing/EDI270.php:L167`, `src/Patient/Cards/InsuranceViewCard.php:L62`, `interface/billing/edi_270.php:L440` |
...
| `x12_default_partner_id` | `sql/database.sql:L3287` | Default trading partner for claims to this payer | `interface/billing/billing_report.php:L1127` |
...
| `inactive` | `sql/database.sql:L3289` | Whether the payer is selectable | Not read on any X12 path |
...
| `x12_default_eligibility_id` | `sql/database.sql:L3291` | Default trading partner for eligibility requests to this payer | `src/Billing/EDI270.php:L412` |
...
**Three payer identifiers plus a fourth, and two default partners: why that combination is a hazard.** VERIFIED: a claim to a given payer is identified by `cms_id` at `src/Billing/Claim.php:L1287`, or by `alt_cms_id` at `src/Billing/Claim.php:L1295` when the encounter-claim flag is set, and is routed to the partner in `x12_default_partner_id` at `interface/billing/billing_report.php:L1127`. An eligibility request to the same payer is identified by `eligibility_id` or `cms_id` at `src/Billing/EDI270.php:L167` depending on a site global, and is routed to the partner in `x12_default_eligibility_id` at `src/Billing/EDI270.php:L412`. Nothing constrains the two default-partner columns to agree, and nothing constrains the identifiers to agree either.

The consequence is that a claim and an eligibility request for the same patient and the same payer can be sent to two different trading partners carrying two different payer identifiers, and both can be correct according to configuration. An eligibility check that succeeds therefore does not establish that a claim to the same payer will reach anyone.

One further hazard is a defect rather than a design consequence, and it is registered as such in [defect-candidates.md](defect-candidates.md) rather than explained here. VERIFIED: in the generation-1 payer model, the accessor pair for the default claims partner reads and writes the payer's receiver identifier property instead of its own, at `library/classes/InsuranceCompany.class.php:L288` and `library/classes/InsuranceCompany.class.php:L293`, and the name lookup that consumes it at `library/classes/InsuranceCompany.class.php:L299` therefore resolves the wrong column. The eligibility equivalents are correct, at `library/classes/InsuranceCompany.class.php:L302-L310`.
...
- [claim-lifecycle.md](claim-lifecycle.md) is where each transaction sits in the revenue cycle: the batch envelope rewriting in stage S5, the acknowledgement counts in stage S7, the eligibility persistence in stage S8, the remittance flush points in stage S10, and what an operator sees when any of them fails.
...
- [defect-candidates.md](defect-candidates.md) registers the suspected defects named throughout, and carries the security appendix that the plaintext interchange password column is flagged into.
